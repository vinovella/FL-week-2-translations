# TODO: Translation updated at 2020-06-27 22:20

translate deutsch strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 1: Stacy"
    new "Tag 1: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle's message"
    new "Tag 2: Squizzle's Nachricht"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Squizzle"
    new "Tag 2: Squizzle"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 2: Polly"
    new "Tag 2: Polly"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 3: Min"
    new "Tag 3: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have made Min cum on day 2"
    new "Du hättest Min an Tag 2 kommen lassen sollen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "2020"
    new "2020"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Daisy and Dahlia"
    new "Party: Daisy und Dahlia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Choose the Secret Oral Santa event"
    new "Wähle das Secret Oral Santa Event"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Party: Mysterious girl"
    new "Party: Mysteriöses Mädchen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: Lydia"
    new "Tag 4: Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 4: AmRose's bathtub"
    new "Tag 4: AmRose's Badewanne"

    # game/code/data/extended/extra_gallery.rpy:15
    old "100 patrons"
    new "100 Patronen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Vanessa"
    new "Tag 5: Vanessa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Lyssa"
    new "Tag 5: Lyssa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have discovered Lyssa's secret"
    new "Du musst Lyssa's Geheimnis entdeckt haben"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Jessie"
    new "Tag 5: Jessie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 5: Min"
    new "Tag 5: Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Give Min private lessons"
    new "Gib Min private Stunden"

    # game/code/data/extended/extra_gallery.rpy:15
    old "200 patrons"
    new "200 Patronen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Karen"
    new "Tag 6: Karen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't creampie Nora"
    new "Bespritze Nora nicht"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa I"
    new "Tag 6: Lyssa I"

    # game/code/data/extended/extra_gallery.rpy:15
    old "You must have chosen Secret Oral Santa but you don't know her secret yet"
    new "Du hättest Secret Oral Santa wählen müssen, aber du kennst ihr Geheimnis noch nicht"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 6: Lyssa II"
    new "Tag 6: Lyssa II"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Stacy"
    new "Tag 7: Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: beach"
    new "Tag 7: Strand"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Don't ask for a release"
    new "Frage nicht nach einer Freilassung"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 7: Polly"
    new "Tag 7: Polly"

# TODO: Translation updated at 2020-07-01 23:25

translate deutsch strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "You have seen all the possible pictures that Maria can send"
    new "Du hast alle möglichen Fotos gesehen, die Maria gesendet haben kann"

# TODO: Translation updated at 2020-07-30 07:17

translate deutsch strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Enter with Eyes Down as ordered"
    new "Tritt mit gesenkten Augen ein, wie angeordnet"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Contact URLittlePony"
    new "Kontaktiere URLittlePony"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Make AmRose your secret girlfriend"
    new "Mache AmRose zu deiner geheimen Freundin"

# TODO: Translation updated at 2020-11-10 12:16

translate deutsch strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 8: AmRose"
    new "Tag 8: AmRose"

# TODO: Translation updated at 2021-08-23 21:58

translate deutsch strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 9: Squizzle - Selfie"
    new "Tag 9: Squizzle - Selfie"

# TODO: Translation updated at 2021-10-13 01:39

translate deutsch strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Ask me one more time"
    new "Frag mich noch einmal"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fill out the questionnaire"
    new "Fülle den Fragebogen aus"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Been there done that"
    new "War da, hab's gemacht"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Complete the Week One Recap"
    new "Schließe den Rückblick der ersten Woche ab"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Double Shower"
    new "Doppelte Dusche"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Take a double shower with Stacy"
    new "Nimm eine doppelte Dusche mit Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "How Bizarre, How Bizarre"
    new "Wie bizarr, wie bizarr"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Tell a very strange story while distracted"
    new "Erzähle zur Ablenkung eine sehr komische Geschichte"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Now I've Seen Everything"
    new "Jetzt habe ich alles gesehen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do all the Samiya's tasks to 100%"
    new "Erledige alle Aufgaben von Samiya zu 100%"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fail Out"
    new "Falle aus"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Screw up on all of Samiya's tasks"
    new "Vermassele alle Aufgaben von Samiya"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Motion Denied"
    new "Antrag abgelehnt"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do not allow Lyssa to achieve completion before Min's party"
    new "Erlaube Lyssa nicht, vor Mins Party fertig zu werden"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Welcome to the party"
    new "Willkommen zur Party"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Play it safe"
    new "Bleib auf der sicheren Seite"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Scotty Doesn't Know"
    new "Scotty weiß es nicht"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Choose and use an absurd safeword"
    new "Wähle und benutze ein absurdes Safeword"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Daisy all the way"
    new "Daisy die ganze Zeit"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do everything you can with Daisy at Min's Party"
    new "Mache bei Min's Party alles, was du mit Daisy tun kannst"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Battleaxe"
    new "Streitaxt"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlock a secret scene using Apples"
    new "Entsperre eine geheime Szene mit Apples"

    # game/code/data/extended/extra_gallery.rpy:15
    old "A walking simulator"
    new "Ein Gehsimulator"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fully explore the Proximity Event"
    new "Erkunde das ganze Nähe Event"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Talk Your Ear Off"
    new "Rede wie ein Wasserfall"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Tell Lydia all the stories"
    new "Erzähle Lydia alle Geschichten"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Outstanding Performance"
    new "Außergewöhnlicher Auftritt"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Complete the Main Event at Min's party in every possible way"
    new "Schließe das Hauptevent bei Min's Party auf jede mögliche Art und Weise ab"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Min Max"
    new "Min Max"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Dump Squizzle for Min"
    new "Schieße Squizzle für Min in den Wind"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Now Serving Hot Pretzels"
    new "Jetzt werden Heiße Bretzeln serviert"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Get entangled with Polly and Nora"
    new "Verwickle dich mit Polly und Nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Coffee and a donut"
    new "Kaffee und ein Donut"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Make Polly's coffee and give AmRose a donut"
    new "Mache Polly's Kaffee und gib AmRose einen Donut"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Doctor McSexy"
    new "Doktor McSexy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Send a girl to the hospital"
    new "Schicke ein Mädchen ins Krankenhaus"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Death By Snu-Snu"
    new "Tod durch Snu-Snu"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Let Dahlia take you towards the light"
    new "Lass dich von Dahlia zum Licht bringen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Hole for Glory"
    new "Loch für Ruhm"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Watch adventures on MariaTV"
    new "Schau dir Abenteuer auf MariaTV an"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Blitzkrieg"
    new "Blitzkrieg"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Trigger two blitz challenges in one playthrough"
    new "Aktiviere zwei Blitzchallenges in einem Durchgang"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Who's Your Daddy?"
    new "Wer ist dein Daddy?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Confirm that you will be a father"
    new "Bestätige, dass du Vater wirst"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Chasing Chastity"
    new "Chastity nachjagen"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Get a special surprise from your girlfriend"
    new "Erhalte eine besondere Überraschung von deiner Freundin"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Eye for an eye"
    new "Auge für Auge"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Make a trade with Terrell"
    new "Handle mit Terrell"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Setec Astronomy"
    new "Setec Astronomie"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Vanessa's little secret"
    new "Vanessa's kleines Geheimnis"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Play hockey with the Law"
    new "Spiele Hockey mit dem Gesetz"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do you have enough quarters?"
    new "Hast du genug Quarters?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "How’s that work?!"
    new "Wie funktioniert das?!"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Deflower Cynthia's freshly renovated garden"
    new "Entblume Cynthia's frisch erneuerten Garten"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Boys are back in town"
    new "Die Jungen sind zurück in der Stadt"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Spend some quality time with Oliver"
    new "Verbringe etwas Zeit mit Oliver"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Why So Serious?"
    new "Warum so ernst?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "View the trio of conflicts during the pool party"
    new "Schaue dem Konflikttrio während der Poolparty zu"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Consistent Bottom"
    new "Immer Bottom"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Submit to everyone"
    new "Untergebe dich jedem"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Always On Top"
    new "Immer oben"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Dominate everyone"
    new "Dominiere jeden"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Sleeping with the Fish"
    new "Schlafen mit dem Fisch"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Answering the Siren’s call"
    new "Dem Ruf der Sirene antworten"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Pretzel Mustard"
    new "Bretzel Senf"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Be with as many people as possible in one playthrough"
    new "Sei mit so vielen, wie möglich in einem Spieldurchgang"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Spicy Pretzel Mustard"
    new "Gewürzte Bretzel mit Senf"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlock all the people on the pool screen"
    new "Entsperre alle Personen auf dem Pool Bildschirm"

    # game/code/data/extended/extra_gallery.rpy:15
    old "True Love, Part 2"
    new "Wahre Liebe, Part 2"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Show Stacy a lot of love!"
    new "Zeige Stacy viel Liebe!"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Between the Thighs of an Angel"
    new "Zwischen den Schenkeln eines Engels"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fall asleep with Lydia in her bed"
    new "Schlafe mit Lydia in ihrem Bett ein"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Due Diligence"
    new "Gebotene Sorgfalt"

    # game/code/data/extended/extra_gallery.rpy:15
    old "See both endings of Week 2"
    new "Schau dir beide Enden von Woche 2 an"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Lowball"
    new "Lowball"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Be with the fewest number of people by the end of the week"
    new "Sei mit den wenigsten Leuten bis zum Ende der Woche"

