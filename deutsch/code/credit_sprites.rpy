translate deutsch strings:

    # game/code/credit_sprites.rpy:7
    old "Thank you for playing the game!"
    new "Danke, dass du das Spiel spielst!"

    # game/code/credit_sprites.rpy:8
    old "We'll see you in Week Three!"
    new "Wir sehen dich in Woche Drei!"

    # game/code/credit_sprites.rpy:14
    old "Story Writing -"
    new "Story schreiben -"

    # game/code/credit_sprites.rpy:21
    old "Art -"
    new "Kunst -"

    # game/code/credit_sprites.rpy:33
    old "Programming -"
    new "Programmierung -"

    # game/code/credit_sprites.rpy:39
    old "Sound design -"
    new "Sound design -"

    # game/code/credit_sprites.rpy:47
    old "Testing & Proofreading -"
    new "Testen & Kontrolllesen -"

    # game/code/credit_sprites.rpy:58
    old "Russian Translation -"
    new "Russische Übersetzung -"

    # game/code/credit_sprites.rpy:63
    old "Italian Translation -"
    new "Italienische Übersetzung -"

    # game/code/credit_sprites.rpy:68
    old "Malay Translation -"
    new "Malaisische Übersetzung -"

    # game/code/credit_sprites.rpy:73
    old "This game was only\nmade possible with\nall the support we have\nfrom Patreon\nand SubscribeStar"
    new "Dieses Spiel wurde nur\nmöglich mit\nall der Unterstützung\nvon Patreon\nund SubscribeStar"

    # game/code/credit_sprites.rpy:74
    old "Huge thanks to\nall of our fans\nand everyone who\nsupported us and\nhelped to bring\nthis story to life"
    new "Ein großes Dankeschön an\nall unsere Fans\nund jeden, der\nuns unterstützt und\nund geholfen hat\ndiese Geschichte zum Leben zu erwecken"

