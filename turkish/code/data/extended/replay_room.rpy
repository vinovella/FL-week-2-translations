translate turkish strings:

    # game/code/data/extended/replay_room.rpy:15
    old "Let Stacy bath with you"
    new "Stacy'nin seninle banyo yapmasına izin ver"

    # game/code/data/extended/replay_room.rpy:15
    old "Let Maria suck you"
    new "Maria'nın seni emmesine izin ver"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 8: URLittlePony"
    new "8. Gün: URLittlePony"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 8: Lyssa Romance"
    new "8. Gün: Romantik Lyssa"

    # game/code/data/extended/replay_room.rpy:15
    old "Give Lyssa a big surprise"
    new "Lyssa'ya büyük bir sürpriz yap"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 8: Lyssa BDSM"
    new "8. Gün: Lyssa BDSM"

    # game/code/data/extended/replay_room.rpy:15
    old "Dominate Lyssa"
    new "Lyssa'ya Hükmet"

    # game/code/data/extended/replay_room.rpy:15
    old "Have some fun first"
    new "Önce biraz eğlenin"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 9: AmRose"
    new "9. Gün: AmRose"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 9: Vanessa"
    new "9. Gün: Vanessa"

    # game/code/data/extended/replay_room.rpy:15
    old "Prank the M.I.L.F"
    new "MILF Şakası"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 9: The Judge"
    new "9. Gün: Yargıç"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 10: Lyssa"
    new "10. Gün: Lyssa"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: Allison"
    new "2. Parti: Allison"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: Cynthia"
    new "2. Parti: Cynthia"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: Antony and Chloe"
    new "2. Parti: Antony ve Chloe"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: AmRose"
    new "2. Parti: AmRose"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: Talia"
    new "2. Parti: Talia"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: Daisy"
    new "2. Parti: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: Dahlia"
    new "2. Parti: Dahlia"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: Lyssa"
    new "2. Parti: Lyssa"

    # game/code/data/extended/replay_room.rpy:15
    old "Proximity"
    new "Yakınlık"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: URLittlePony"
    new "2. Parti: URLittlePony"

    # game/code/data/extended/replay_room.rpy:15
    old "Horseback Riding"
    new "Binicilik"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: Call with Lydia"
    new "2. Parti: Lydia ile Görüşme"

    # game/code/data/extended/replay_room.rpy:15
    old "Party 2: Allison and Squizzle"
    new "2. Parti: Allison ve Squizzle"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 11: Lyssa & Allison"
    new "11. Gün: Lyssa ve Allison"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 11: Polly & Nora"
    new "11. Gün: Polly ve Nora"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 11: AmRose"
    new "11. Gün: AmRose"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 12: AmRose & Maria"
    new "12. Gün: AmRose ve Maria"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 12: Hana"
    new "12. Gün: Hana"

    # game/code/data/extended/replay_room.rpy:15
    old "Do Hana a favor"
    new "Hana'ya bir iyilik yap"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 12: The Judge"
    new "12. Gün: Yargıç"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 12: Lyssa Romance"
    new "12. Gün: Romantik Lyssa"

    # game/code/data/extended/replay_room.rpy:15
    old "Be a considerate lover"
    new "Düşünceli bir sevgili ol"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 12: Lyssa BDSM"
    new "12. Gün: Lyssa BDSM"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 12: Lydia"
    new "12. Gün: Lydia"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 12: Daisy"
    new "12. Gün: Daisy"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 12: Dahlia"
    new "12. Gün: Dahlia"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 13: Chloe & Kevin"
    new "12. Gün: Chloe ve Kevin"

    # game/code/data/extended/replay_room.rpy:15
    old "Study later, fun first"
    new "Sonra çalış, önce eğlence"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 13: Maria"
    new "13. Gün: Maria"

    # game/code/data/extended/replay_room.rpy:15
    old "Ask her to send photos"
    new "Ondan fotoğraf göndermesini iste"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 13: Iona"
    new "13. Gün: Iona"

    # game/code/data/extended/replay_room.rpy:15
    old "Don't be a hero"
    new "Kahraman olma"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 13: Aaleyah"
    new "13. Gün: Aaleyah"

    # game/code/data/extended/replay_room.rpy:15
    old "Do a trade"
    new "Ticaret yap"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 13: Stacy"
    new "13. Gün: Stacy"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 14: AmRose"
    new "14. Gün: AmRose"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 14: Min and Lydia"
    new "14. Gün: Min ve Lydia"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 14: Cynthia"
    new "14. Gün: Cynthia"

    # game/code/data/extended/replay_room.rpy:15
    old "Play along"
    new "Birlikte oynayın"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 14: Polly"
    new "14. Gün: Polly"

    # game/code/data/extended/replay_room.rpy:15
    old "Don't creampie Nora"
    new "Nora'ya boşalma"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 14: Allison"
    new "14. Gün: Allison"

    # game/code/data/extended/replay_room.rpy:15
    old "Go with her"
    new "Onunla git"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 14: Samia"
    new "14. Gün: Samia"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 14: Oliver"
    new "14. Gün: Oliver"

    # game/code/data/extended/replay_room.rpy:15
    old "Help him out"
    new "Ona yardım et"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 14: Jessie Mermaid"
    new "14. Gün: Jessie Deniz Kızı"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 14: Lydia"
    new "14. Gün: Lydia"

    # game/code/data/extended/replay_room.rpy:15
    old "Day 14: Stacy"
    new "14. Gün: Stacy"

