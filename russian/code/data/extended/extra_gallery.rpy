# TODO: Translation updated at 2021-10-15 09:58

translate russian strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Ask me one more time"
    new "Спроси меня еще раз"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fill out the questionnaire"
    new "Заполни анкету"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Been there done that"
    new "Был там, делал это"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Complete the Week One Recap"
    new "Заверши прохождение первой недели"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Double Shower"
    new "Двойной Душ"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Take a double shower with Stacy"
    new "Прими двойной душ вместе со Стейси"

    # game/code/data/extended/extra_gallery.rpy:15
    old "How Bizarre, How Bizarre"
    new "Как Странно, Как Странно"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Tell a very strange story while distracted"
    new "Расскажи очень странную историю"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Now I've Seen Everything"
    new "Теперь я все повидал"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do all the Samiya's tasks to 100%"
    new "Выполняй все задания Самии на 100%%"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fail Out"
    new "Потерпеть неудачу"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Screw up on all of Samiya's tasks"
    new "Облажаться во всех заданиях Самии"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Motion Denied"
    new "Ходатайство отклонено"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do not allow Lyssa to achieve completion before Min's party"
    new "Не позволяйте Лиссе кончить до вечеринки Мин"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Welcome to the party"
    new "Добро пожаловать на вечеринку"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Play it safe"
    new "Играть безопасно"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Scotty Doesn't Know"
    new "Скотти не знает"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Choose and use an absurd safeword"
    new "Выберите и используйте абсурдное стоп-слово"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Daisy all the way"
    new "Дейзи навсегда"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do everything you can with Daisy at Min's Party"
    new "Сделай все возможное вместе с Дейзи на вечеринке Мин"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Battleaxe"
    new "Боевой топор"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlock a secret scene using Apples"
    new "Разблокируй секретную сцену с помощью Яблок"

    # game/code/data/extended/extra_gallery.rpy:15
    old "A walking simulator"
    new "Симулятор ходьбы"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fully explore the Proximity Event"
    new "Полностью изучи событие близости"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Talk Your Ear Off"
    new "Займи её уши"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Tell Lydia all the stories"
    new "Расскажи Лидии все истории"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Outstanding Performance"
    new "Выполнение служебных обязанностей"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Complete the Main Event at Min's party in every possible way"
    new "Заверши главное событие на вечеринке Мин всеми возможными способами"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Min Max"
    new "Мин максимально"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Dump Squizzle for Min"
    new "Используй Сквизли и Мин"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Now Serving Hot Pretzels"
    new "Подаем горячие штучки"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Get entangled with Polly and Nora"
    new "Свяжись с Полли и Норой"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Coffee and a donut"
    new "Кофе и печенька"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Make Polly's coffee and give AmRose a donut"
    new "Сделай кофе Полли и дай АмРоуз печеньку"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Doctor McSexy"
    new "Доктор Секси"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Send a girl to the hospital"
    new "Отправь девушку в больницу"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Death By Snu-Snu"
    new "Смерть От Сну-Сну"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Let Dahlia take you towards the light"
    new "Позволь Далии вести тебя к свету"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Hole for Glory"
    new "Дыра славы"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Watch adventures on MariaTV"
    new "Смотри приключения на МарияTV"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Blitzkrieg"
    new "Блицкриг"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Trigger two blitz challenges in one playthrough"
    new "Запусти два блиц-задания в одном прохождении"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Who's Your Daddy?"
    new "Кто тут папочка?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Confirm that you will be a father"
    new "Подтверди, что станешь отцом"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Chasing Chastity"
    new "В погоне за целомудрием"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Get a special surprise from your girlfriend"
    new "Получи особый сюрприз от своей девушки"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Eye for an eye"
    new "Око за око"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Make a trade with Terrell"
    new "Заключи сделку с Терреллом"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Setec Astronomy"
    new "Астрономия"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Vanessa's little secret"
    new "Маленький секрет Ванессы"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Play hockey with the Law"
    new "Играйте в хоккей с законом"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do you have enough quarters?"
    new "У вас достаточно четвертаков?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "How’s that work?!"
    new "Как это работает?!"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Deflower Cynthia's freshly renovated garden"
    new "Лишить девственности недавно отремонтированный сад Синтии"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Boys are back in town"
    new "Мальчики вернулись в город"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Spend some quality time with Oliver"
    new "Проведи немного времени с Оливером"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Why So Serious?"
    new "Почему так серьезно?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "View the trio of conflicts during the pool party"
    new "Посмотри на три конфликта во время вечеринки у бассейна"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Consistent Bottom"
    new "Последовательное дно"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Submit to everyone"
    new "Подчинись всем"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Always On Top"
    new "Всегда на вершине"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Dominate everyone"
    new "Доминируй над всеми"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Sleeping with the Fish"
    new "Секс с рыбой"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Answering the Siren’s call"
    new "Ответь на зов Сирены"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Pretzel Mustard"
    new "Крендель с горчицей"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Be with as many people as possible in one playthrough"
    new "Потрахайся с как можно большим количеством людей в одном прохождении"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Spicy Pretzel Mustard"
    new "Крендель с острой горчицей"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlock all the people on the pool screen"
    new "Разблокируй всех на экране бассейна"

    # game/code/data/extended/extra_gallery.rpy:15
    old "True Love, Part 2"
    new "Настоящая Любовь, Часть 2"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Show Stacy a lot of love!"
    new "Покажи Стейси свою любовь!"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Between the Thighs of an Angel"
    new "Между ног у Ангела"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fall asleep with Lydia in her bed"
    new "Засни с Лидией в ее постели"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Due Diligence"
    new "Должное усердие"

    # game/code/data/extended/extra_gallery.rpy:15
    old "See both endings of Week 2"
    new "Увидь обе концовки Недели 2"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Lowball"
    new "Пуританин"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Be with the fewest number of people by the end of the week"
    new "Закончи неделю с наименьшим количеством людей"

