
translate russian strings:

    # game/code/data/prologue.rpy:16
    old "Scintillating Stacy"
    new "Блистательная Стейси"

    # game/code/data/prologue.rpy:16
    old "The sister"
    new "Сестра"

    # game/code/data/prologue.rpy:16
    old "The close childhood friend"
    new "Близкий друг детства"

    # game/code/data/prologue.rpy:16
    old "Playful Polly"
    new "Игривая Полли"

    # game/code/data/prologue.rpy:16
    old "The pink-haired pansexual exhibitionist"
    new "Розоволосая пансексуальная эксгибиционистка"

    # game/code/data/prologue.rpy:16
    old "Voluptuous Vanessa"
    new "Сладострастная Ванесса"

    # game/code/data/prologue.rpy:16
    old "The M.I.L.F."
    new "М.И.Л.Ф.А."

    # game/code/data/prologue.rpy:16
    old "Charming Chloe"
    new "Очаровательная Хлоя"

    # game/code/data/prologue.rpy:16
    old "The village bicycle"
    new "Деревенская велосипедка"

    # game/code/data/prologue.rpy:16
    old "Jovial Jessie"
    new "Веселая Джесси"

    # game/code/data/prologue.rpy:16
    old "The cosplayer extraordinaire"
    new "Экстраординарная косплеерша"

    # game/code/data/prologue.rpy:16
    old "Magnificent Min"
    new "Великолепная Мин"

    # game/code/data/prologue.rpy:16
    old "Obsessed with winning & watersports"
    new "Одержима победой в водных видах спорта"

    # game/code/data/prologue.rpy:16
    old "Lovely Lyssa"
    new "Прекрасная Лисса"

    # game/code/data/prologue.rpy:16
    old "The sexy lawyer with a secret surprise"
    new "Сексуальный адвокат с тайным сюрпризом"

    # game/code/data/prologue.rpy:16
    old "Maidenly Maria"
    new "Девственная Мария"

    # game/code/data/prologue.rpy:16
    old "The creep stalker lesbian"
    new "Жуткая лесбиянка-сталкер"

    # game/code/data/prologue.rpy:16
    old "Delicious Daisy"
    new "Восхитительная Дейзи"

    # game/code/data/prologue.rpy:16
    old "The submissive free spirit"
    new "Покорный свободный дух"

    # game/code/data/prologue.rpy:16
    old "Dazzling Dahlia"
    new "Ослепительная Далия"

    # game/code/data/prologue.rpy:16
    old "The dominatrix"
    new "Госпожа"

    # game/code/data/prologue.rpy:34
    old "Cumshot"
    new "Камшот"

    # game/code/data/prologue.rpy:34
    old "Creampie"
    new "Кремпай"

    # game/code/data/prologue.rpy:34
    old "Enema"
    new "Клизма"

    # game/code/data/prologue.rpy:34
    old "Watersports"
    new "Водные виды спорта"

    # game/code/data/prologue.rpy:34
    old "Footfetish"
    new "Футфетиш"

    # game/code/data/prologue.rpy:34
    old "Anal play"
    new "Анальная игра"

    # game/code/data/prologue.rpy:34
    old "Pegging"
    new "Связывание"

    # game/code/data/prologue.rpy:50
    old "Not_Mike"
    new "Не_Майк"


translate russian strings:

    # game/code/data/prologue.rpy:34
    old "Trans Content"
    new "Транс-контент"

