# TODO: Translation updated at 2021-11-14 14:09

# game/code/d08/d08s03.rpy:16
translate russian d08s03_4ee8cc8f:

    # "9:00 AM"
    "9:00 утра"

# game/code/d08/d08s03.rpy:23
translate russian d08s03_23c37cdf:

    # "Ok, AmRose is already here."
    "Хорошо, АмРоуз уже здесь."

# game/code/d08/d08s03.rpy:32
translate russian d08s03_014606bc:

    # mc "Hey Min, Antony, are you hosting another... ?"
    mc "Привет Мин, Энтони, планируете следующую вечеринку...?"

# game/code/d08/d08s03.rpy:35
translate russian d08s03_7c8ad1b9:

    # mes "Hey [mcname]!"
    mes "Привет [mcname]!"

# game/code/d08/d08s03.rpy:37
translate russian d08s03_476d3299:

    # mes "Yes. We're going to have another little get together this Wednesday night."
    mes "Да. У нас будет еще одна небольшая вечеринка в эту среду вечером."

# game/code/d08/d08s03.rpy:40
translate russian d08s03_ffd853cf:

    # am "Yes, of course! This Wednesday night we'll host another little soiree!"
    am "Да, конечно! В эту среду мы проведем еще одну небольшую вечеринку!"

# game/code/d08/d08s03.rpy:43
translate russian d08s03_20ef59dd:

    # mes "Shhhhh. {w}Don't yell it out to everybody."
    mes "Шшшшш. {w}Не кричи об этом при всех."

# game/code/d08/d08s03.rpy:44
translate russian d08s03_23d05496:

    # am "Sorry, lady love."
    am "Простите, моя леди."

# game/code/d08/d08s03.rpy:45
translate russian d08s03_38fc3e2b:

    # mc "Oh good, good. I wasn't sure how to ask."
    mc "Хорошо, хорошо. Я просто не знал, как спросить."

# game/code/d08/d08s03.rpy:47
translate russian d08s03_f61da1d5:

    # mes "I hope you'll be there."
    mes "Надеюсь, ты там тоже будешь."

# game/code/d08/d08s03.rpy:49
translate russian d08s03_0b4516c9:

    # mc "Is that an invitation?"
    mc "Это что, приглашение?"

# game/code/d08/d08s03.rpy:51
translate russian d08s03_e8924c39:

    # mes "Hmm... {w}No."
    mes "Хмм... {w}Нет."

# game/code/d08/d08s03.rpy:53
translate russian d08s03_2a513d79:

    # am "As much as we'd love to invite you and a few others, we're trying to..."
    am "Как бы мы ни хотели пригласить тебя и некоторых других, мы не можем..."

# game/code/d08/d08s03.rpy:54
translate russian d08s03_9875b977:

    # mes "We don't want to give away invites, especially to people who can earn their own way."
    mes "Мы не должны раздавать приглашения, особенно тем, кто может заработать сам."

# game/code/d08/d08s03.rpy:56
translate russian d08s03_ee47e49d:

    # mc "But if you're sure that I can get enough points for the Party, why not just invite me?"
    mc "Но если ты уверена, что я смогу получить достаточно очков для приглашения на Вечеринку, то почему бы тебе просто не пригласить меня?"

# game/code/d08/d08s03.rpy:60
translate russian d08s03_7dfaf170:

    # mes "Where's the fun in that?"
    mes "А где в этом веселье?"

# game/code/d08/d08s03.rpy:62
translate russian d08s03_87b23c5f:

    # am "Go out there and score points the old fashioned way."
    am "Иди и набирай очки по старинке."

# game/code/d08/d08s03.rpy:65
translate russian d08s03_c85aafcd:

    # am "Where's the fun in that?"
    am "А где в этом веселье?"

# game/code/d08/d08s03.rpy:66
translate russian d08s03_1acabea5:

    # mes "Go out there and earn points just like we do."
    mes "Иди и зарабатывай очки, как и мы."

# game/code/d08/d08s03.rpy:68
translate russian d08s03_9fa7debd:

    # mes "Besides, the more people that pay with points, the more money we get from the app, the better the next Party will be."
    mes "Кроме того, чем больше людей заплатят очками, тем больше денег мы получим от приложения, тем лучше будет следующая Вечеринка."

# game/code/d08/d08s03.rpy:70
translate russian d08s03_8e4d8272:

    # am "Shhhh. {w}Don't tell anyone, but this week is going to be EPIC."
    am "Шшшшш. {w}Никому не говори, но эта неделя обещает быть ЭПИЧНОЙ."

# game/code/d08/d08s03.rpy:71
translate russian d08s03_15570c18:

    # mc "Cool, cool. Looking forward to it."
    mc "Круто. С нетерпением буду ждать этого."

# game/code/d08/d08s03.rpy:74
translate russian d08s03_e3da725d:

    # mes "See ya after class."
    mes "Увидимся после занятий."

# game/code/d08/d08s03.rpy:76
translate russian d08s03_05500ff6:

    # mc "Huh?"
    mc "Что?"

# game/code/d08/d08s03.rpy:77
translate russian d08s03_b19c39f0:

    # am "We've got some planning to do here. See ya later, okay?"
    am "У нас тут кое-какие планы. Увидимся позже, хорошо?"

# game/code/d08/d08s03.rpy:78
translate russian d08s03_1126c76e:

    # mc "Sure, yeah. See ya."
    mc "Конечно, да. Увидимся."

# game/code/d08/d08s03.rpy:85
translate russian d08s03_699878f6:

    # mc "Hey!"
    mc "Привет!"

# game/code/d08/d08s03.rpy:87
translate russian d08s03_c25c2fcc:

    # arj "Are you joining?"
    arj "Узнал что нибудь?"

# game/code/d08/d08s03.rpy:91
translate russian d08s03_b01dd5cf:

    # mc "The Party will be Wednesday night."
    mc "Вечеринка будет в среду вечером."

# game/code/d08/d08s03.rpy:92
translate russian d08s03_ef789455:

    # mc "Not sure about the details yet, but there should be something on the app soon."
    mc "Пока не уверен насчет деталей, но скоро что-нибудь появится в приложении."

# game/code/d08/d08s03.rpy:94
translate russian d08s03_62169626:

    # arj "Cool, thanks."
    arj "Хорошо, спасибо."

# game/code/d08/d08s03.rpy:97
translate russian d08s03_aa721223:

    # mc "Are you doing okay? I got a message last night..."
    mc "Ты в порядке? Вчера вечером я получил сообщение..."

# game/code/d08/d08s03.rpy:100
translate russian d08s03_ccf766c6:

    # arj "What kind of message?"
    arj "Какое сообщение?"

# game/code/d08/d08s03.rpy:101
translate russian d08s03_43410c56:

    # mc "From that thing we can't talk about."
    mc "От этой штуки, о которой мы не можем говорить."

# game/code/d08/d08s03.rpy:104
translate russian d08s03_68dabad6:

    # arj "Oh. He-who-shall-not-be-named."
    arj "Ох. Тот-кого-нельзя-называть."

# game/code/d08/d08s03.rpy:106
translate russian d08s03_b6668550:

    # mc "Heh, yeah. I guess we could call it Tom Riddle."
    mc "Хех, да. Думаю, мы могли бы называть его Том Риддл."

# game/code/d08/d08s03.rpy:108
translate russian d08s03_3175ad62:

    # arj "What did he say?"
    arj "И что он сказал?"

# game/code/d08/d08s03.rpy:109
translate russian d08s03_315dea22:

    # mc "Just that you didn't have enough points and were going to fail."
    mc "Только то, что у тебя не хватает очков и ты потерпишь неудачу."

# game/code/d08/d08s03.rpy:112
translate russian d08s03_d3fceaab:

    # arj "Is that why you sent me a ton of points this morning?"
    arj "Так вот почему ты прислал мне кучу очков?"

# game/code/d08/d08s03.rpy:113
translate russian d08s03_fceb4365:

    # mc "Yeah, it wouldn't tell me how short you were, so I just tried to guess."
    mc "Да, мне не сказали, сколько тебе не хватает, так что я просто попытался угадать."

# game/code/d08/d08s03.rpy:115
translate russian d08s03_01a3e0cd:

    # arj "That's really sweet. Remind me to thank you later."
    arj "Очень мило. Напомни мне поблагодарить тебя позже."

# game/code/d08/d08s03.rpy:117
translate russian d08s03_0c2e0e96:

    # arj "Is that why you sent me some points this morning?"
    arj "Поэтому ты прислал мне несколько очков?"

# game/code/d08/d08s03.rpy:118
translate russian d08s03_fceb4365_1:

    # mc "Yeah, it wouldn't tell me how short you were, so I just tried to guess."
    mc "Да, мне не сказали, сколько тебе не хватает, так что я просто попытался угадать."

# game/code/d08/d08s03.rpy:120
translate russian d08s03_18466d64:

    # arj "That's kinda sweet."
    arj "Как мило."

# game/code/d08/d08s03.rpy:121
translate russian d08s03_98fef643:

    # mc "So, are you in danger?"
    mc "Так ты в опасности?"

# game/code/d08/d08s03.rpy:124
translate russian d08s03_c9bb4b8c:

    # arj "Oh, no. I was never in any danger."
    arj "Ох, нет. Я никогда не была в опасности."

# game/code/d08/d08s03.rpy:126
translate russian d08s03_3d84fa54:

    # arj "I was going to use my Gold Star, but you sent me so many points that I didn't have to do that."
    arj "Я собиралась использовать свою Золотую Звезду, но ты прислал мне столько очков, что мне не пришлось этого делать."

# game/code/d08/d08s03.rpy:128
translate russian d08s03_e06398e6:

    # arj "I just used my Gold Star."
    arj "Я просто использовала свою Золотую Звезду."

# game/code/d08/d08s03.rpy:131
translate russian d08s03_8d0e4425:

    # mc "What's a Gold Star?"
    mc "Что такое Золотая Звезда?"

# game/code/d08/d08s03.rpy:133
translate russian d08s03_b29d08e1:

    # arj "If you win a daily challenge and you're in \"the program\", you get a Gold Star. {w}I won that daily challenge last Thursday, so..."
    arj "Если выигрываешь ежедневное задание и находишься в \"Программе\", то получаешь Золотую Звезду. {w}Я выиграла это ежедневное задание в прошлый четверг, так что..."

# game/code/d08/d08s03.rpy:134
translate russian d08s03_d661624e:

    # mc "Okay, but what's a Gold Star?"
    mc "Хорошо, но что такое Золотая Звезда?"

# game/code/d08/d08s03.rpy:136
translate russian d08s03_c18f900e:

    # arj "Oh! You can use it like a \"Get out of jail free\" to pass on a goal that you otherwise would have failed."
    arj "О! Её можно использовать как \"Освобождение от наказания\", чтобы избежать наказания, который в противном случае настигло бы меня."

# game/code/d08/d08s03.rpy:138
translate russian d08s03_876c474c:

    # mc "Oh, so if I win a Gold Star, then I can stop playing for a few days?"
    mc "О, значит если я выиграю Золотую Звезду, то смогу перестать играть на несколько дней?"

# game/code/d08/d08s03.rpy:141
translate russian d08s03_120b18d2:

    # arj "Exactly. That's why I wasn't worrying about doing anything over the weekend."
    arj "Именно так. Вот почему я не беспокоилась о том, чтобы заработать очки в выходные."

# game/code/d08/d08s03.rpy:142
translate russian d08s03_58aba63d:

    # mc "Good to know. Thanks."
    mc "Хорошо это знать. Спасибо."

# game/code/d08/d08s03.rpy:145
translate russian d08s03_72b9536c:

    # mc "Oh, right. I totally forgot you won that challenge last week."
    mc "А, ну да. Я совсем забыла, что ты выиграл это задание на прошлой неделе."

# game/code/d08/d08s03.rpy:147
translate russian d08s03_68799ee0:

    # arj "Boof!"
    arj "Буф!"

# game/code/d08/d08s03.rpy:148
translate russian d08s03_f36e5aec:

    # mc "Exactly."
    mc "Вот именно."

# game/code/d08/d08s03.rpy:162
translate russian d08s03_9e4cf501:

    # arj "Anyway, class is going to start soon."
    arj "Впрочем, скоро начнётся урок."

# game/code/d08/d08s03.rpy:164
translate russian d08s03_415e490a:

    # mc "Alright, I'm gonna go chat with Kevin."
    mc "Ладно, пойду поболтаю с Кевином."

# game/code/d08/d08s03.rpy:168
translate russian d08s03_254d4ec2:

    # mct "Weird. Maria is staring at me with crazy stalker eyes."
    mct "Странно. Мария смотрит на меня безумными глазами сталкера."

# game/code/d08/d08s03.rpy:172
translate russian d08s03_ea9b9792:

    # arj "Don't be a stranger."
    arj "Ещё увидимся."

# game/code/d08/d08s03.rpy:178
translate russian d08s03_87200cb2:

    # mc "Hey Kev, how was your weekend?"
    mc "Привет, Кев, как прошли выходные?"

# game/code/d08/d08s03.rpy:180
translate russian d08s03_2d975032:

    # kb "A-maz-ing. You wouldn't even believe what I got up to yesterday if I told you."
    kb "От-лич-но. Ты не поверишь, что я вчера вытворял."

# game/code/d08/d08s03.rpy:181
translate russian d08s03_9a1c051d:

    # mct "Considering what I learned about him last week... {w}Yeah, he's probably right."
    mct "Учитывая то, что я узнал о нем на прошлой неделе... {w}Да, наверное, он прав."

# game/code/d08/d08s03.rpy:183
translate russian d08s03_12d3c264:

    # kb "What about you, [mcname]? Good weekend?"
    kb "А ты, [mcname]? Хорошо провёл выходные?"

# game/code/d08/d08s03.rpy:185
translate russian d08s03_d5222758:

    # mc "Great weekend. You might not believe it."
    mc "Отлично провёл. Ты тоже не не поверишь."

# game/code/d08/d08s03.rpy:186
translate russian d08s03_573312da:

    # kb "Try me."
    kb "Попробуй рассказать."

# game/code/d08/d08s03.rpy:188
translate russian d08s03_b7f9c574:

    # mc "Lydia and I had our first date on Saturday."
    mc "В субботу у нас с Лидией было первое свидание."

# game/code/d08/d08s03.rpy:189
translate russian d08s03_92e7f101:

    # kb "You mean her concert at the coffee shop, where she jumped off the stage and kissed you in front of everyone?"
    kb "Ты имеешь в виду ее концерт в кафе, где она спрыгнула со сцены и поцеловала тебя на глазах у всех?"

# game/code/d08/d08s03.rpy:191
translate russian d08s03_5c829999:

    # mc "Exactly. It was so amazing."
    mc "Вот именно. Это было так замечательно."

# game/code/d08/d08s03.rpy:192
translate russian d08s03_61320ef8:

    # mc "She's got this perfect voice and afterwards we just sat there talking for hours..."
    mc "У нее такой прекрасный голос, а потом мы просто сидели и разговаривали..."

# game/code/d08/d08s03.rpy:195
translate russian d08s03_bdaffafe:

    # kb "I know, dude. I was there."
    kb "Я знаю, чувак. Я был там."

# game/code/d08/d08s03.rpy:196
translate russian d08s03_7234564e:

    # mc "Oh, shit. That's right. {w}I kinda forgot anyone else was there."
    mc "Ох, черт. Точно. {w}Я вроде как забыл, что там были и другие."

# game/code/d08/d08s03.rpy:197
translate russian d08s03_457ae91c:

    # mc "It felt like she was singing just for me."
    mc "Такое ощущение, что она пела только для меня."

# game/code/d08/d08s03.rpy:199
translate russian d08s03_43c3ded7:

    # kb "Yeah, but the room was full of people."
    kb "Да, но там было полно людей."

# game/code/d08/d08s03.rpy:205
translate russian d08s03_53c693da:

    # mct "What the hell? Does she want something from me?"
    mct "Какого черта? Она что-то от меня хочет?"

# game/code/d08/d08s03.rpy:208
translate russian d08s03_4b062766:

    # mc "Fine. So, yeah, then yesterday I was at the beach."
    mc "Хорошо. И ещё я вчера был на пляже."

# game/code/d08/d08s03.rpy:209
translate russian d08s03_e104beb9:

    # kb "For that kayaking competition? How was it?"
    kb "Для того соревнования по каякингу? Ну и как там?"

# game/code/d08/d08s03.rpy:210
translate russian d08s03_f658d5bd:

    # mc "Let's just say that I didn't see any of the kayaking."
    mc "Скажем так, я не видел ни одного каяка."

# game/code/d08/d08s03.rpy:212
translate russian d08s03_848aa8ff:

    # kb "Nice. {w}Ah, shit. Looks like the lecture is about to start."
    kb "Класс. {w}Ах, черт. Похоже, лекция вот-вот начнется."

# game/code/d08/d08s03.rpy:214
translate russian d08s03_a6be913e:

    # mc "So?"
    mc "И что?"

# game/code/d08/d08s03.rpy:217
translate russian d08s03_db230f4d:

    # kb "Seriously, we both need to buckle up..."
    kb "Серьезно, нам обоим нужно пристегнуться..."

# game/code/d08/d08s03.rpy:218
translate russian d08s03_be86f2ed:

    # mc "...buckle in?"
    mc "...пристегнуться?"

# game/code/d08/d08s03.rpy:220
translate russian d08s03_b706c3fc:

    # kb "We both need to buckle something and pay attention if we're going to be ready for finals."
    kb "Нам обоим нужно пристегнутся и обратить внимание, если мы хотим быть готовыми к экзамену."

# game/code/d08/d08s03.rpy:221
translate russian d08s03_a42ad403:

    # mc "Yeah, I guess you're right."
    mc "Да, наверное, ты прав."

# game/code/d08/d08s03.rpy:228
translate russian d08s03_51142325:

    # rn "Today, class, we will be discussing the history of..."
    rn "Сегодня, класс, мы будем обсуждать историю..."

# game/code/d08/d08s03.rpy:229
translate russian d08s03_1d9b9f55:

    # mct "blah blah blah"
    mct "Бла бла бла."

# game/code/d08/d08s03.rpy:237
translate russian d08s03_ed7c6e73:

    # mct "Is Maria looking at me again, or has she been staring at me this whole time?"
    mct "Мария снова смотрит на меня, или она все это время пялилась на меня?"

# game/code/d08/d08s03.rpy:242
translate russian d08s03_c24affd4:

    # mct "Huh. It's a message from Squizzle."
    mct "Хм. Сообщение от Сквизли."

# game/code/d08/d08s03.rpy:243
translate russian d08s03_665c43c1:

    # mct "I wonder why Min isn't just texting me - she knows who I am."
    mct "Интересно, почему Мин просто не напишет мне - она же знает, кто я."

# game/code/d08/d08s03.rpy:244
translate russian d08s03_cdb4c145:

    # mct "Oh, right. She's got that whole thing where Squizzle can do things Min never would."
    mct "А, ну да. Это Сквизли может делать то, что Мин никогда бы не сделала."

# game/code/d08/d08s03.rpy:251
translate russian d08s03_9426416c:

    # mct "Squizzle can get her pussy eaten out without cheating on her boyfriend."
    mct "Сквизли может позволить вылизать ее киску, не изменяя своему парню."

# game/code/d08/d08s03.rpy:257
translate russian d08s03_bd242c3d:

    # mct "Squizzle and I can strip naked and swap pee without cheating on her boyfriend."
    mct "Сквизли может раздеться догола и поиграться мочой, не изменяя своему парню."

# game/code/d08/d08s03.rpy:263
translate russian d08s03_12871f42:

    # mct "Squizzle can have my dick in her mouth without cheating on her boyfriend."
    mct "Сквизли может брать члены в рот, не изменяя своему парню."

# game/code/d08/d08s03.rpy:269
translate russian d08s03_30440b27:

    # mct "Squizzle can tongue fuck my mouth while we sloppily swap mouthfuls of piss without cheating on her boyfriend."
    mct "Сквизли может языком трахать мой рот, пока мы небрежно обмениваемся мочой, не изменяя своему парню."

# game/code/d08/d08s03.rpy:272
translate russian d08s03_afccbe42:

    # mct "Squizzle could probably... {w}anyway, let's see what she wants."
    mct "Сквизли наверное, может... {w}ладно, посмотрим, чего она хочет."

# game/code/d08/d08s03.rpy:273
translate russian d08s03_da13a8f7:

    # mct "Oh, she wants me to meet her after class... {w}at the Men's Room?!"
    mct "Она хочет, чтобы я встретился с ней после занятий... {w}в мужском туалете?!"

# game/code/d08/d08s03.rpy:274
translate russian d08s03_030896a6:

    # mct "This has to be related to the pissing, doesn't it?"
    mct "Это должно быть связано с мочёй, верно?"

# game/code/d08/d08s03.rpy:275
translate russian d08s03_a0c15eb0:

    # mct "Hmm. I'm supposed to be her pee guru. Her master of urination."
    mct "Хмм. Я должно быть стал ее писающим гуру. Ее учителем уринации."

# game/code/d08/d08s03.rpy:276
translate russian d08s03_5ce36734:

    # mct "I doubt that I can find a watersports course curriculum online."
    mct "Я сомневаюсь, что смогу найти учебную программу по водным видам спорта в интернете."

# game/code/d08/d08s03.rpy:281
translate russian d08s03_e1fde363:

    # rn "Excuse me - did I miss something?"
    rn "Простите - я что-то пропустил?"

# game/code/d08/d08s03.rpy:283
translate russian d08s03_8e21e123:

    # rn "Miss Jenkins. Amber-Rose, would you like to tell me why everyone in my class suddenly has their phones out?"
    rn "Мисс Дженкинс. Амбер-Роуз, не хотите рассказать мне, почему все в моем классе внезапно уставились в телефоны?"

# game/code/d08/d08s03.rpy:286
translate russian d08s03_ab0e4d14:

    # arj "No, sir. I'm sorry."
    arj "Нет, сэр. Извините."

# game/code/d08/d08s03.rpy:288
translate russian d08s03_b69f5a07:

    # rn "Very well. Mr. Bennet. Kevin, is there some urgent news? Maybe a terrorist attack?"
    rn "Очень хорошо. Мистер Беннет. Кевин, есть какие-то срочные новости? Может быть, террористический акт?"

# game/code/d08/d08s03.rpy:291
translate russian d08s03_99f98c53:

    # kb "No, sir."
    kb "Нет, сэр."

# game/code/d08/d08s03.rpy:293
translate russian d08s03_609af294:

    # rn "Perhaps the building is on fire? Some celebrity spotted without their underwear?"
    rn "Может быть, где-то горит здание? Или какую-то знаменитость заметили без нижнего белья?"

# game/code/d08/d08s03.rpy:295
translate russian d08s03_3e6aff8e:

    # kb "No. It's just... {w}It's nothing like that. Just some game."
    kb "Нет. Это просто... {w}Просто игра."

# game/code/d08/d08s03.rpy:297
translate russian d08s03_d14c87d9:

    # rn "Any objections if I continue with class? Fine, good. Let's continue."
    rn "Есть возражения, если я продолжу занятия? Хорошо. Давайте продолжим."

# game/code/d08/d08s03.rpy:298
translate russian d08s03_d3a88b30:

    # rn "During the Battle of Columbus in 1865..."
    rn "Во время битвы при Колумбусе в 1865 году..."

# game/code/d08/d08s03.rpy:299
translate russian d08s03_87048d51:

    # mct "Wow, that was uncomfortable."
    mct "Вау, это было некомфортно."

# game/code/d08/d08s03.rpy:302
translate russian d08s03_84949172:

    # mct "Alright, let's see what everyone else was already looking at."
    mct "Ладно, нужно посмотреть, на что смотрели все остальные."

# game/code/d08/d08s03.rpy:303
translate russian d08s03_51214afc:

    # mct "Fetish Locator, of course. What's today's challenge?"
    mct "Фетиш-Локатор, конечно же. В чем заключается сегодняшняя задание?"

# game/code/d08/d08s03.rpy:304
translate russian d08s03_69e79a88:

    # fl "From pressing lips to deep throat, gulp it down or spit it out."
    fl "От прикосновения к губам до глубины горла, проглоти или выплюнь."

# game/code/d08/d08s03.rpy:305
translate russian d08s03_018e5924:

    # fl "The daily challenge for today is Blow Job."
    fl "Ежедневное задание на сегодня - это Минет."

# game/code/d08/d08s03.rpy:306
translate russian d08s03_c5c7c1f2:

    # mct "Huh. Last Monday was cum. Today is a blowjob. I guess Mondays are for men."
    mct "Хм. В прошлый понедельник была сперма. Сегодня - минет. Думаю, понедельники для мужчин."

# game/code/d08/d08s03.rpy:307
translate russian d08s03_6a5b7574:

    # mct "Okay. Buckling down. Focusing on the lecture."
    mct "Ладно. Не отвлекатся. Сосредоточимся на лекции."

# game/code/d08/d08s03.rpy:313
translate russian replay_d08s03_e3ebb20d:

    # mct "Is Maria still looking at me?"
    mct "Мария все еще смотрит на меня?"

# game/code/d08/d08s03.rpy:317
translate russian replay_d08s03_bd5eaa7b:

    # mct "Okay, this is just crazy. What the hell does she want?"
    mct "Ладно, это просто безумие. Какого черта она хочет?"

# game/code/d08/d08s03.rpy:321
translate russian replay_d08s03_3240527e:

    # rn "*blah blah blah*... Wilson's Raid into Georgia...*blah blah blah*"
    rn "*Бла бла бла*... Нападение Уилсона на Джорджию... *бла бла бла*."

# game/code/d08/d08s03.rpy:323
translate russian replay_d08s03_8907d4f5:

    # mc "You've been staring at me ever since you walked in..."
    mc "Ты пялишься на меня с тех пор, как вошла..."

# game/code/d08/d08s03.rpy:326
translate russian replay_d08s03_48438ce3:

    # mk "Sorry, sorry, sorry. Can you sit with me?"
    mk "Прости, прости, прости. Можешь посидеть со мной?"

# game/code/d08/d08s03.rpy:329
translate russian replay_d08s03_9b10bfe5:

    # mc "I'm here now. What do you want?"
    mc "Я здесь. Чего ты хочешь?"

# game/code/d08/d08s03.rpy:331
translate russian replay_d08s03_61d87077:

    # mk "I keep thinking about that event at the Party."
    mk "Я все думаю о том мероприятии на Вечеринке."

# game/code/d08/d08s03.rpy:332
translate russian replay_d08s03_dd56af82:

    # mc "Huh? Antony's magic show?"
    mc "Хм? Волшебное шоу Энтони?"

# game/code/d08/d08s03.rpy:334
translate russian replay_d08s03_dc7b255c:

    # mk "Yes. That dream we shared... you know, when you fucked me."
    mk "Да. То шоу, который мы разделяли... знаешь, когда ты трахал меня."

# game/code/d08/d08s03.rpy:335
translate russian replay_d08s03_84f16542:

    # mc "Okay..."
    mc "И..."

# game/code/d08/d08s03.rpy:337
translate russian replay_d08s03_144dbbdf:

    # mk "Before that, when I was sucking your cock... {w}it was just the best thing ever."
    mk "До этого, когда я сосала твой член... {w}это была просто лучшая вещь на свете."

# game/code/d08/d08s03.rpy:339
translate russian replay_d08s03_a03597fd:

    # mc "I've got some time this afternoon. Should I drop by-"
    mc "У меня есть немного времени сегодня днем. Может мне заскочить..."

# game/code/d08/d08s03.rpy:344
translate russian replay_d08s03_4172a42a:

    # mk "I want it now."
    mk "Я хочу этого сейчас."

# game/code/d08/d08s03.rpy:346
translate russian replay_d08s03_cc761816:

    # mc "No..."
    mc "Нет..."

# game/code/d08/d08s03.rpy:349
translate russian replay_d08s03_060cdf91:

    # mk "Yes. Now."
    mk "Да. Сейчас."

# game/code/d08/d08s03.rpy:350
translate russian replay_d08s03_76ab189f:

    # mc "Seriously? Right here, right now?"
    mc "Серьезно? Прямо здесь, прямо сейчас?"

# game/code/d08/d08s03.rpy:352
translate russian replay_d08s03_e1337149:

    # mk "I know we're in class, but I can't help it."
    mk "Я знаю, что мы в классе, но ничего не могу с собой поделать."

# game/code/d08/d08s03.rpy:354
translate russian replay_d08s03_18a295dc:

    # mk "I just kept thinking about sucking your cock. {w}I need it."
    mk "Я все время думаю о том, чтобы пососать твой член. {w}Мне это нужно."

# game/code/d08/d08s03.rpy:355
translate russian replay_d08s03_37daaa62:

    # mct "Wow. I know it's wrong, but maybe I should have some fun with her."
    mct "Вау. Я знаю, что это неправильно, но, может быть, мне стоит немного позабавиться с ней."

# game/code/d08/d08s03.rpy:358
translate russian replay_d08s03_ba026827:

    # mc "Maybe I can find someone to help you with that."
    mc "Может быть, я найду кого-нибудь, кто поможет тебе с этим."

# game/code/d08/d08s03.rpy:359
translate russian replay_d08s03_2479a251:

    # mc "Maybe the Professor would like-"
    mc "Может быть, профессор..."

# game/code/d08/d08s03.rpy:361
translate russian replay_d08s03_9efac0f9:

    # mk "No! {w}Shh... shit... no. I need {b}your{/b} cock."
    mk "Нет! {w}Шшш... черт... нет. Мне нужен {b}твой{/b} член."

# game/code/d08/d08s03.rpy:364
translate russian replay_d08s03_6f90ed59:

    # mc "Fine. Take it out."
    mc "Хорошо. Вытащи его."

# game/code/d08/d08s03.rpy:370
translate russian replay_d08s03_b363fc51:

    # mc "Maria. Listen here. You think that you are missing out on something but that is not the case."
    mc "Мария. Слушай сюда. Ты думаешь, что упускаешь что-то, но это не так."

# game/code/d08/d08s03.rpy:371
translate russian replay_d08s03_36723146:

    # mc "We just had a crazy experience at the party, but that wasn't real."
    mc "У нас был сумасшедший опыт на вечеринке, но это было не по-настоящему."

# game/code/d08/d08s03.rpy:372
translate russian replay_d08s03_9df17b07:

    # mc "I think you should calm down and finally decide what is that you actually want. Ok?"
    mc "Думаю, тебе стоит успокоиться и наконец-то решить, чего ты на самом деле хочешь. Хорошо?"

# game/code/d08/d08s03.rpy:375
translate russian replay_d08s03_61f1d479:

    # mk "Wow. I didn't expect this from you. You might be right."
    mk "Вау. Я не ожидала этого от тебя. Возможно, ты прав."

# game/code/d08/d08s03.rpy:381
translate russian replay_d08s03_e37de9e5:

    # mk "What?"
    mk "Что?"

# game/code/d08/d08s03.rpy:384
translate russian replay_d08s03_ab4411c4:

    # mc "Take it out and feel it in your hand."
    mc "Вытащи его и почувствуй в руке."

# game/code/d08/d08s03.rpy:387
translate russian replay_d08s03_76101272:

    # mk "I can't believe I'm doing this."
    mk "Не могу поверить, что делаю это."

# game/code/d08/d08s03.rpy:388
translate russian replay_d08s03_f7339009:

    # mct "I can't believe she's doing this either!"
    mct "Я тоже не могу поверить, что она это делает!"

# game/code/d08/d08s03.rpy:391
translate russian replay_d08s03_7e59b137:

    # mc "That's it. Now give me a handjob."
    mc "Вот так. А теперь подрочи мне."

# game/code/d08/d08s03.rpy:393
translate russian replay_d08s03_71d54b6e:

    # mk "I don't... {w}How?"
    mk "Я не знаю... {w}Как?"

# game/code/d08/d08s03.rpy:394
translate russian replay_d08s03_c1c99e06:

    # mc "You've played with a dildo, right? You've fucked yourself with toys like that?"
    mc "Ты ведь играла с дилдо, да? Ты трахала себя такими игрушками?"

# game/code/d08/d08s03.rpy:396
translate russian replay_d08s03_02916c9b:

    # mk "Uhhhh huh... Yes."
    mk "Уххх, хм... Да."

# game/code/d08/d08s03.rpy:398
translate russian replay_d08s03_cb1e089a:

    # mc "Just pretend that your hand is your cunt and stroke me off."
    mc "Просто представь, что твоя рука - твоя пизда, и подрочи мне."

# game/code/d08/d08s03.rpy:400
translate russian replay_d08s03_cd004a74:

    # mk "I don't know what that means..."
    mk "Я не совсем понимаю как это сделать..."

# game/code/d08/d08s03.rpy:402
translate russian replay_d08s03_0adc01c5:

    # mc "I think you do. You're already doing it."
    mc "Думаю понимаешь. Ты уже это делаешь."

# game/code/d08/d08s03.rpy:403
translate russian replay_d08s03_374d8e06:

    # mk "What? {w}Oh, I guess I am."
    mk "Что? {w}Ох, наверное, так оно и есть."

# game/code/d08/d08s03.rpy:404
translate russian replay_d08s03_40ca58e2:

    # mc "Don't just stroke the shaft. You have to work the tip too."
    mc "Не надо просто теребить ствол. Ты также должна работать с головкой."

# game/code/d08/d08s03.rpy:408
translate russian replay_d08s03_e8d3d303:

    # mc "Just pretend that it is a really large clit."
    mc "Просто представь, что это большой клитор."

# game/code/d08/d08s03.rpy:410
translate russian replay_d08s03_dcf2226b:

    # mc "Work the tip of my cock with your hand like you would finger some sexy girl's clit."
    mc "Поработай над головкой моего члена своей рукой так, как будто бы ты пальцем поглаживаешь клитор какой-нибудь сексуальной девчонки."

# game/code/d08/d08s03.rpy:413
translate russian replay_d08s03_06c05314:

    # mk "Uhhhhhuh... Yes, yes..."
    mk "Уххххххух... Да, да..."

# game/code/d08/d08s03.rpy:414
translate russian replay_d08s03_72e19af2:

    # mc "That feels good. Now faster."
    mc "Приятное ощущение. Теперь быстрее."

# game/code/d08/d08s03.rpy:427
translate russian replay_d08s03_cb46ca00:

    # mk "Do... Do you like it?"
    mk "Так... Тебе нравится?"

# game/code/d08/d08s03.rpy:429
translate russian replay_d08s03_9696be6f:

    # mc "Now, let's change things up a little."
    mc "А теперь давай немного изменим ситуацию."

# game/code/d08/d08s03.rpy:430
translate russian replay_d08s03_784cf9ff:

    # mk "What do you mean?"
    mk "Что ты имеешь в виду?"

# game/code/d08/d08s03.rpy:431
translate russian replay_d08s03_daf5cfa4:

    # mc "Open your mouth, place it over my hard cock, and lick the tip while you stroke the shaft."
    mc "Открой рот, возьми в него мой твердый член и облизывай головку, пока дрочишь ствол."

# game/code/d08/d08s03.rpy:436
translate russian replay_d08s03_0062d401:

    # mk "Yesss... yess... okay."
    mk "Дааа... даа... Хорошо."

# game/code/d08/d08s03.rpy:438
translate russian replay_d08s03_2041a1f8:

    # mct "Wow! Is she really going to-"
    mct "Вау! Она действительно делает это..."

# game/code/d08/d08s03.rpy:448
translate russian replay_d08s03_1892ce08:

    # mct "Damn! This is not like in the dream..."
    mct "Черт возьми! Это совсем не то, что было тогда..."

# game/code/d08/d08s03.rpy:456
translate russian replay_d08s03_58c7d629:

    # mct "In the dream it felt like heaven. But in real life it just feels like she's never given head before..."
    mct "Тогда это было похоже на рай. Но сейчас кажется, что ей никогда раньше не давали в рот..."

# game/code/d08/d08s03.rpy:464
translate russian replay_d08s03_a027693c:

    # mct "She's even scraping my dick with her teeth a bit..."
    mct "Она даже немного царапает мне член зубами..."

# game/code/d08/d08s03.rpy:471
translate russian replay_d08s03_ad69a398:

    # mct "But I have to give her an A for the effort! She is trying hard!"
    mct "Но нужно отдать ей должное! Она очень старается!"

# game/code/d08/d08s03.rpy:475
translate russian replay_d08s03_cdd613b4:

    # rn "blah blah blah saber wound to the chest blah blah blah"
    rn "Бла бла бла сабельное ранение в грудь бла бла бла..."

# game/code/d08/d08s03.rpy:484
translate russian replay_d08s03_601170d1:

    # mc "I'm about to cum."
    mc "Я вот-вот кончу."

# game/code/d08/d08s03.rpy:486
translate russian replay_d08s03_4d5c738d:

    # mc "Maria. I'm going to cum."
    mc "Мария. Я сейчас кончу."

# game/code/d08/d08s03.rpy:488
translate russian replay_d08s03_2227ff25:

    # mct "Fuck it. I guess she wants me to cum in her mouth."
    mct "Да похуй. Наверное, она хочет, чтобы я кончил ей в рот."

# game/code/d08/d08s03.rpy:497
translate russian replay_d08s03_1def35e7:

    # mct "Oh hell yes."
    mct "Ох, черт возьми, да."

# game/code/d08/d08s03.rpy:504
translate russian replay_d08s03_283292cb:

    # mct "Holy shit - she's not stopping."
    mct "Срань господня - она не останавливается."

# game/code/d08/d08s03.rpy:510
translate russian replay_d08s03_b1c9f928:

    # rn "Achem!"
    rn "Кхм!"

# game/code/d08/d08s03.rpy:517
translate russian replay_d08s03_bb989d63:

    # rn "Mr. Young. [mcname]."
    rn "Мистер Янг. [mcname]."

# game/code/d08/d08s03.rpy:520
translate russian replay_d08s03_6934998d:

    # mc "Huh? Yes, sir?"
    mc "Что? Да, сэр?"

# game/code/d08/d08s03.rpy:522
translate russian replay_d08s03_dd68f8a3:

    # rn "Why was it that Pemberton mixed up that first tonic of cocaine and caffeine?"
    rn "Зачем Пембертон смешал первый тоник из кокаина и кофеина?"

# game/code/d08/d08s03.rpy:525
translate russian replay_d08s03_cbf695c8:

    # mct "Shit... umm... I know this..."
    mct "Черт... эмм... Я же знаю это..."

# game/code/d08/d08s03.rpy:537
translate russian replay_d08s03_59df0524:

    # rn "That is correct. It's nice to see that someone did the reading."
    rn "Верно. Приятно видеть, что хоть кто-то читает."

# game/code/d08/d08s03.rpy:539
translate russian replay_d08s03_cb1d5c9e:

    # rn "Not even close. Has anyone here actually done the reading?"
    rn "Даже не близко. Кто-нибудь здесь вообще читал?"

# game/code/d08/d08s03.rpy:540
translate russian replay_d08s03_3c3bc6c8:

    # rn "Miss Eun-Soo. Min, do you know the correct answer?"
    rn "Мисс Ен-Су. Мин, ты знаешь правильный ответ?"

# game/code/d08/d08s03.rpy:543
translate russian replay_d08s03_f18f651f:

    # mes "Due to his injury he became addicted to morphine, and was trying to get clean."
    mes "Из-за травмы он стал принимать морфий и стал зависимым. Таким способом он пытался избавится от зависимости."

# game/code/d08/d08s03.rpy:544
translate russian replay_d08s03_2501a982:

    # rn "Exactly what I've come to expect from you. That is correct."
    rn "Именно такой ответ я и ожидал. Всё верно."

# game/code/d08/d08s03.rpy:548
translate russian replay_d08s03_5f4aaeeb:

    # rn "Miss Kovalevich. Maria, is everything alright?"
    rn "Мисс Ковалевич. Мария, с вами все в порядке?"

# game/code/d08/d08s03.rpy:551
translate russian replay_d08s03_44d5e6fa:

    # mk "Yes, sir. I had just dropped my pen."
    mk "Да, сэр. Я просто уронила ручку."

# game/code/d08/d08s03.rpy:553
translate russian replay_d08s03_9e26a891:

    # rn "Of course. Your classmate already mentioned it was a treatment for drug addiction."
    rn "Конечно. Тут уже упоминалось, что это было лечение от наркомании."

# game/code/d08/d08s03.rpy:554
translate russian replay_d08s03_d2097d76:

    # rn "What else was French Wine Coca used as a treatment for?"
    rn "От чего еще использовалась французская винная Кока?"

# game/code/d08/d08s03.rpy:557
translate russian replay_d08s03_3a5fb0f1:

    # mk "Oh. {w}Depression, alcoholism, and neurasthenia."
    mk "Ох. {w}От депрессии, алкоголизма и неврастении."

# game/code/d08/d08s03.rpy:559
translate russian replay_d08s03_b7bed3c8:

    # rn "And would you remind us what the nickname for neurasthenia was?"
    rn "И не могли бы вы напомнить нам, как называлась неврастения?"

# game/code/d08/d08s03.rpy:561
translate russian replay_d08s03_c67668e6:

    # mk "Yes, sir. It was called Americanitis."
    mk "Да, сэр. Она называлась Американитус."

# game/code/d08/d08s03.rpy:563
translate russian replay_d08s03_16986113:

    # rn "Very good. You may go back to finding your pen if you want."
    rn "Отлично. Можете вернуться к поиску своей ручки."

# game/code/d08/d08s03.rpy:564
translate russian replay_d08s03_9cb414c8:

    # mct "Oh shit. Anyone who didn't notice what she was doing before..."
    mct "Вот дерьмо. Раньше никто не обращал внимания, что она делала..."

# game/code/d08/d08s03.rpy:567
translate russian replay_d08s03_7ae47b07:

    # mk "Shit shit shit shit shit..."
    mk "Черт черт черт черт черт..."

# game/code/d08/d08s03.rpy:568
translate russian replay_d08s03_acd30f0e:

    # mc "I think some people might have noticed us..."
    mc "Думаю, что некоторые могли заметить..."

# game/code/d08/d08s03.rpy:575
translate russian replay_d08s03_34764a38:

    # mk "Fuck fuck fuck fuck fuck..."
    mk "Черт черт черт черт черт..."

# game/code/d08/d08s03.rpy:579
translate russian replay_d08s03_b592930e:

    # mct "Oh, fuck!"
    mct "Ох, черт!"

# game/code/d08/d08s03.rpy:580
translate russian replay_d08s03_91ac509f:

    # mct "How many people just texted me?"
    mct "Сколько людей только что написали мне?"

# game/code/d08/d08s03.rpy:581
translate russian replay_d08s03_a6491393:

    # mct "\"How did you get a lesbian to suck your dick in class?\""
    mct "\"Как ты заставил лесбиянку сосать свой член в классе?\""

# game/code/d08/d08s03.rpy:583
translate russian replay_d08s03_31b58985:

    # mct "\"I thought she was a dyke!\""
    mct "\"Я думал, что она лесбиянка!\""

# game/code/d08/d08s03.rpy:584
translate russian replay_d08s03_a93d015c:

    # mct "\"Did you just turn her bisexual?\""
    mct "\"Ты только что превратил ее в бисексуалку?\""

# game/code/d08/d08s03.rpy:585
translate russian replay_d08s03_f487f6e7:

    # mct "The list goes on and on..."
    mct "Сообщения всё продолжают приходить..."

# game/code/d08/d08s03.rpy:587
translate russian replay_d08s03_322d7aba:

    # mk "*whispers* Fuck me sideways with a sledgehammer."
    mk "*шепчет* Трахни меня своей кувалдой."

# game/code/d08/d08s03.rpy:588
translate russian replay_d08s03_b68ac449:

    # mct "Oh shit, this has to be a thousand times worse for her."
    mct "Вот дерьмо, сказать такое должно быть в тысячу раз сложнее для нее."

# game/code/d08/d08s03.rpy:590
translate russian replay_d08s03_5a0d9fea:

    # mc "Maria, fuck. I'm so sorry. I didn't-"
    mc "Мария, черт. Мне так жаль. Я не..."

# game/code/d08/d08s03.rpy:592
translate russian replay_d08s03_bdb152de:

    # mk "Shh. Just... shhh."
    mk "Шшш. Просто... шшшш."

# game/code/d08/d08s03.rpy:604
translate russian d08s03_end_8bac5718:

    # "Some time later"
    "Через некоторое время."

# game/code/d08/d08s03.rpy:607
translate russian d08s03_end_2c1cb7ed:

    # rn "Alright, class. I think that is enough for today."
    rn "Хорошо, класс. Думаю, на сегодня достаточно."

# game/code/d08/d08s03.rpy:611
translate russian d08s03_end_6bf3ab4e:

    # mct "Alright, time to meet Squizzle at the toilet."
    mct "Ладно, пора встретится с Сквизли в туалете."

# game/code/d08/d08s03.rpy:613
translate russian d08s03_end_11796383:

    # mct "Alright, I should meet up with Stacy and start our Samiya hunt."
    mct "Ладно, я должен встретиться со Стейси и начать нашу охоту на Самию."

translate russian strings:

    # game/code/d08/d08s03.rpy:362
    old "Fine"
    new "Ладно"

    # game/code/d08/d08s03.rpy:362
    old "You don't want it"
    new "Ты этого не хочешь"

    # game/code/d08/d08s03.rpy:526
    old "He was creating an energy drink"
    new "Он создавал энергетический напиток"

    # game/code/d08/d08s03.rpy:526
    old "To sell an addictive drink to make lots of money"
    new "Он создавал вызывающий зависимость напиток, чтобы заработать много денег"

    # game/code/d08/d08s03.rpy:526
    old "He was a morphine addict trying to get clean"
    new "Он был морфийным наркоманом и пытался снять зависимость"

    # game/code/d08/d08s03.rpy:526
    old "It was an accident"
    new "Это был несчастный случай"

