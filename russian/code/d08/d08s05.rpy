# TODO: Translation updated at 2020-11-04 21:11

# game/code/d08/d08s05.rpy:5
translate russian d08s05_06a950b5:

    # "11:00 AM"
    "11:00 утра"

# game/code/d08/d08s05.rpy:8
translate russian d08s05_0c829b89:

    # sy "So, this is the University? I might have made the wrong choice for my higher education."
    sy "Так это и есть университет? Кажется, я сделала неправильный выбор в пользу своего высшего образования."

# game/code/d08/d08s05.rpy:9
translate russian d08s05_f25d77a6:

    # mc "It is nice, but our college will look better on your resume after we graduate."
    mc "Он красивый, но наш колледж будет выглядеть лучше в твоём резюме после того, как ты закончишь его."

# game/code/d08/d08s05.rpy:11
translate russian d08s05_7bfd3d68:

    # sy "So, now that we're here... {w}What's the plan?"
    sy "Итак, теперь, когда мы здесь... {w}Какой у нас план?"

# game/code/d08/d08s05.rpy:12
translate russian d08s05_62eb5652:

    # mc "Are you sure you want to go through with this?"
    mc "Ты уверена, что хочешь пройти через это?"

# game/code/d08/d08s05.rpy:14
translate russian d08s05_938b7516:

    # sy "We wouldn't have come all this way if I wasn't."
    sy "Иначе мы бы не проделали весь этот путь."

# game/code/d08/d08s05.rpy:15
translate russian d08s05_e74eeec9:

    # mc "I told you what Samiya is like - what she did to the Prom King and Prom Queen."
    mc "Я уже говорил тебе, кто такая Самия - что она сделала с королем и королевой выпускного бала."

# game/code/d08/d08s05.rpy:17
translate russian d08s05_7e30384d:

    # sy "I just want my stuff back. How are we getting it?"
    sy "Я просто хочу вернуть свои вещи. Как мы их получим?"

# game/code/d08/d08s05.rpy:18
translate russian d08s05_e673201b:

    # mc "We are looking for Dahlia. She should be around here."
    mc "Мы ищем Далию. Она должна быть где-то здесь."

# game/code/d08/d08s05.rpy:20
translate russian d08s05_64c9fce9:

    # sy "Why do we need her?"
    sy "Зачем она нам нужна?"

# game/code/d08/d08s05.rpy:21
translate russian d08s05_59ce15e9:

    # mc "Because she seems to be the one who knows Samiya."
    mc "Потому что она единственная, кто знает Самию."

# game/code/d08/d08s05.rpy:23
translate russian d08s05_6d30b682:

    # sy "Dahlia goes to this University?"
    sy "Далия учится в этом университете?"

# game/code/d08/d08s05.rpy:24
translate russian d08s05_880d2687:

    # mc "Yes, but remember that according to Dahlia it isn't worth it..."
    mc "Да, но помни, что, по мнению Далии, это того не стоит..."

# game/code/d08/d08s05.rpy:26
translate russian d08s05_6bfc74d9:

    # sy "What?"
    sy "Что?"

# game/code/d08/d08s05.rpy:27
translate russian d08s05_dce25133:

    # mc "She told me that it would be better to forget about the luggage and move on."
    mc "Она сказала мне, что лучше забыть о багаже и двигаться дальше."

# game/code/d08/d08s05.rpy:29
translate russian d08s05_d315eb8c:

    # sy "If this Samiya stabs you and sets me on fire you have my permission to say, \"I told you so.\""
    sy "Если эта Самия ударит тебя и подожжет меня, я разрешаю тебе сказать: \"Я же тебе говорил\"."

# game/code/d08/d08s05.rpy:30
translate russian d08s05_180348a2:

    # mc "Let's hope it doesn't come to that."
    mc "Будем надеяться, что до этого не дойдет."

# game/code/d08/d08s05.rpy:49
translate russian d08s05_dd_fd8a6781:

    # mc "So - see that woman over there? That's Daisy."
    mc "Вон, видишь ту девушку? Это Дейзи."

# game/code/d08/d08s05.rpy:51
translate russian d08s05_dd_ba59fc42:

    # sy "You know her?"
    sy "Ты ее знаешь?"

# game/code/d08/d08s05.rpy:53
translate russian d08s05_dd_a87a337b:

    # mc "Yep."
    mc "Ага."

# game/code/d08/d08s05.rpy:57
translate russian d08s05_dd_91980308:

    # mc "Hi Daisy!"
    mc "Привет, Дейзи!"

# game/code/d08/d08s05.rpy:59
translate russian d08s05_dd_6fd4a291:

    # dd "Hey there, Little Mister Big Cock! Who's your friend?"
    dd "Привет, Маленький Мистер Большой Член! Кто твоя подруга?"

# game/code/d08/d08s05.rpy:61
translate russian d08s05_dd_1e6c78c1:

    # sy "Little mister big cock?"
    sy "Маленький мистер большой член?"

# game/code/d08/d08s05.rpy:63
translate russian d08s05_dd_08d512fc:

    # mc "Haha... um..."
    mc "Хаха... эм..."

# game/code/d08/d08s05.rpy:65
translate russian d08s05_dd_1e653888:

    # mc "Daisy, have you met my sister?"
    mc "Дейзи, ты не знакома с моей сестрой?"

# game/code/d08/d08s05.rpy:67
translate russian d08s05_dd_ab4a12cd:

    # dd "Oh, this is your landlady's youngest tenant?"
    dd "А, это самая молодая арендатор твоей квартирной хозяйки?"

# game/code/d08/d08s05.rpy:68
translate russian d08s05_dd_ede96a43:

    # sy "Um... no? {w}Hi Daisy, I'm [mcname]'s sister."
    sy "Эм... нет? {w}Привет, Дейзи, я сестра [mcname]."

# game/code/d08/d08s05.rpy:70
translate russian d08s05_dd_7cf82f6d:

    # mc "Daisy, this is my best friend from back home."
    mc "Дейзи, это моя лучшая подруга из дома."

# game/code/d08/d08s05.rpy:72
translate russian d08s05_dd_cf7150be:

    # dd "Oh! I didn't mean to embarrass you in front of your special lady friend."
    dd "Ох! Я не хотела ставить тебя в неловкое положение перед твоей лучшей подругой."

# game/code/d08/d08s05.rpy:73
translate russian d08s05_dd_e9af3fbe:

    # mc "She's not my-"
    mc "Она не моя..."

# game/code/d08/d08s05.rpy:74
translate russian d08s05_dd_abae32e1:

    # sy "Um... no. {w}Hi Daisy, [mcname] and I have been friends since like forever."
    sy "Эм... нет? {w}Привет Дейзи, [mcname] и я дружим уже почти целую вечность."

# game/code/d08/d08s05.rpy:76
translate russian d08s05_dd_9a7079ba:

    # sy "My name is Stacy."
    sy "Меня зовут Стейси."

# game/code/d08/d08s05.rpy:78
translate russian d08s05_dd_df4260e7:

    # dd "Pleasure to meet you. Are you two touring the campus? Thinking about coming over here for next semester?"
    dd "Приятно познакомиться. Гуляете по кампусу? Думаешь приехать сюда на следующий семестр?"

# game/code/d08/d08s05.rpy:80
translate russian d08s05_dd_03ed6f44:

    # sy "Not exactly."
    sy "Не совсем."

# game/code/d08/d08s05.rpy:82
translate russian d08s05_dd_f0639b31:

    # mc "We were actually hoping to see you."
    mc "Вообще-то, мы надеялись увидеть тебя."

# game/code/d08/d08s05.rpy:84
translate russian d08s05_dd_5b5f17af:

    # dd "You came all the way over here for me? I'm touched."
    dd "Ты проделал весь этот путь ради меня? Я тронута."

# game/code/d08/d08s05.rpy:85
translate russian d08s05_dd_6c0f74f9:

    # mc "Well, not you exactly."
    mc "Ну, не совсем ради тебя."

# game/code/d08/d08s05.rpy:87
translate russian d08s05_dd_320ef5c0:

    # sy "We're looking for Samiya, or Dahlia to help us finding Samiya."
    sy "Мы ищем Самию. Или Далию, которая может помочь нам найти ее."

# game/code/d08/d08s05.rpy:88
translate russian d08s05_dd_51e70fe8:

    # mc "And hoping to find out more about her, or find people who know her."
    mc "И надеемся узнать о ней побольше, или найти людей, которые ее знают."

# game/code/d08/d08s05.rpy:90
translate russian d08s05_dd_a53f1da1:

    # dd "Oh, I don't really know Samiya..."
    dd "Ох, я не знаю Самию..."

# game/code/d08/d08s05.rpy:92
translate russian d08s05_dd_14041de3:

    # mc "But you surely know Dah-"
    mc "Но ты наверняка знаешь где Да..."

# game/code/d08/d08s05.rpy:93
translate russian d08s05_dd_0b6586b9:

    # dd "I mean, we were close back in high school, but don't really see each other much since we came to University."
    dd "Я имею в виду, мы были близки в старших классах, но не часто видимся с тех пор, как поступили в университет."

# game/code/d08/d08s05.rpy:95
translate russian d08s05_dd_5ef43fce:

    # sy "Huh?"
    sy "Что?"

# game/code/d08/d08s05.rpy:96
translate russian d08s05_dd_1651f315:

    # dd "Yeah, you know how it is. You get to University and make different friends and-"
    dd "Да, знаете, как это бывает. Поступаешь в университет и заводишь разных друзей и..."

# game/code/d08/d08s05.rpy:98
translate russian d08s05_dd_b292b593:

    # sy "So, you do know her?"
    sy "Так ты ее знаешь?"

# game/code/d08/d08s05.rpy:99
translate russian d08s05_dd_02c7b059:

    # dd "Well, yeah. Sorta."
    dd "Ну, да. Типа того."

# game/code/d08/d08s05.rpy:101
translate russian d08s05_dd_1d7fc9ca:

    # mc "You went to high school with Samiya? And Dahlia too?"
    mc "Ты училась в школе с Самией? И Далия тоже?"

# game/code/d08/d08s05.rpy:103
translate russian d08s05_dd_7aa3a666:

    # dd "It's not that weird. Like half the people in our graduating class came here."
    dd "Это не так уж и странно. Половина выпускников из нашего класса пришли сюда."

# game/code/d08/d08s05.rpy:105
translate russian d08s05_dd_2cd36bd2:

    # sy "Can you tell us anything about Samiya?"
    sy "Ты можешь рассказать нам что-нибудь о Самии?"

# game/code/d08/d08s05.rpy:106
translate russian d08s05_dd_79500dd1:

    # dd "Oh sure! {w}Let's see..."
    dd "Ну конечно! {w}Давай посмотрим..."

# game/code/d08/d08s05.rpy:109
translate russian d08s05_dd_a3ea74e4:

    # dd "Dahlia and Samiya never got along."
    dd "Далия и Самия никогда между собой не ладили."

# game/code/d08/d08s05.rpy:110
translate russian d08s05_dd_ff251ef7:

    # dd "Samiya would prank Dahlia, so Dahlia would talk shit about Samiya, so Samiya would prank Dahlia..."
    dd "Самия разыгрывала Далию, из-за этого Далия говорила всякое о Самии, поэтому Самия разыгрывала Далию..."

# game/code/d08/d08s05.rpy:112
translate russian d08s05_dd_3859fa41:

    # dd "Meanwhile I was there, friends with both of them, and just like \"Ugh, you're as bad as Batman and the Joker. Just fuck already!\""
    dd "Я была там, дружила с ними обоими, и это было похоже на: \"Тьфу, ты такая же плохая, как Бэтмен и Джокер.\""

# game/code/d08/d08s05.rpy:114
translate russian d08s05_dd_24fb4913:

    # sy "What?!"
    sy "Что?!"

# game/code/d08/d08s05.rpy:116
translate russian d08s05_dd_67db1ff0:

    # dd "Batman and the Joker. Harry and Draco? Statler and Waldorf?"
    dd "Бэтмен и Джокер. Гарри и Драко?"

# game/code/d08/d08s05.rpy:117
translate russian d08s05_dd_b92a4d87:

    # mc "Did they ever...?"
    mc "Они когда-нибудь...?"

# game/code/d08/d08s05.rpy:119
translate russian d08s05_dd_e57db9cf:

    # dd "Gah! No. {w}It got even worse after Dahlia's boyfriend's tires got slashed."
    dd "Нет. {w}Всё стало еще хуже после того, как она порезала шины парню Далии."

# game/code/d08/d08s05.rpy:121
translate russian d08s05_dd_a34281ac:

    # mc "Was that the Prom King that Dahlia was talking about? The one that rejected Samiya?"
    mc "Это был тот самый король выпускного бала, о котором говорила Далия? Тот, что отверг Самию?"

# game/code/d08/d08s05.rpy:123
translate russian d08s05_dd_f5eb4f21:

    # dd "Ha! Oh, yeah. There's a funny story about that."
    dd "Ха! О, да. Об этом есть забавная история."

# game/code/d08/d08s05.rpy:125
translate russian d08s05_dd_42576e57:

    # sy "How's that?"
    sy "Какая?"

# game/code/d08/d08s05.rpy:127
translate russian d08s05_dd_136263e1:

    # dd "Okay, Dahlia started dating this guy. This whole alpha male sports jock guy. On the football team and everything."
    dd "Ладно, Далия начала встречаться с этим парнем. Весь из себя альфа-самец-спортивный качок. В футбольной команде и все такое."

# game/code/d08/d08s05.rpy:129
translate russian d08s05_dd_f1f628d2:

    # dd "And so Samiya thought Dahlia was being so phony."
    dd "И поэтому Самия решила, что Далия такая лживая."

# game/code/d08/d08s05.rpy:131
translate russian d08s05_dd_560e2665:

    # dd "Like, we weren't bad girls, but we weren't trying to be Prom Queen or anything."
    dd "Типа, мы не были плохими девочками, но мы не пытались быть королевой выпускного или что-то в этом роде."

# game/code/d08/d08s05.rpy:133
translate russian d08s05_dd_757d136e:

    # sy "So, Samiya slashed the guy's tires?"
    sy "Значит, Самия порезала парню шины?"

# game/code/d08/d08s05.rpy:135
translate russian d08s05_dd_6ea4ab56:

    # dd "Oh, no. Definitely not."
    dd "О, нет. Определенно нет."

# game/code/d08/d08s05.rpy:136
translate russian d08s05_dd_90226541:

    # mc "Oh, good."
    mc "Ох, хорошо."

# game/code/d08/d08s05.rpy:138
translate russian d08s05_dd_df5dcf96:

    # dd "I mean, she definitely had someone else do it for her."
    dd "Я хочу сказать, она заставила кого-то другого сделать это за нее."

# game/code/d08/d08s05.rpy:139
translate russian d08s05_dd_f9365191:

    # mc "Oh, of course."
    mc "Ох, ну конечно."

# game/code/d08/d08s05.rpy:141
translate russian d08s05_dd_010d4f6d:

    # dd "Anyway, so Dahlia put on this big campaign for Prom Queen, even though everybody knew she wouldn't win."
    dd "Так или иначе, Далия затеяла большую кампанию за звание королевы выпускного бала, хотя все знали, что она не выиграет."

# game/code/d08/d08s05.rpy:142
translate russian d08s05_dd_b15e19b8:

    # sy "That's kinda sad."
    sy "Это довольно грустно."

# game/code/d08/d08s05.rpy:144
translate russian d08s05_dd_43fe5a5b:

    # dd "Then when... What was her name? Britney? Barbie? Brandi? Some popular ditz with a B- name."
    dd "Потом, когда... Как ее звали? Бритни? Барби? Бренди? Кто-то популярная с именем начинающееся на Б."

# game/code/d08/d08s05.rpy:145
translate russian d08s05_dd_d7e1eca5:

    # mc "Doesn't matter."
    mc "Неважно."

# game/code/d08/d08s05.rpy:147
translate russian d08s05_dd_19729ae1:

    # dd "Right, so she becomes Prom Queen."
    dd "Точно, значит, она стала королевой бала."

# game/code/d08/d08s05.rpy:149
translate russian d08s05_dd_d58a287d:

    # sy "Dahlia?"
    sy "Далия?"

# game/code/d08/d08s05.rpy:151
translate russian d08s05_dd_052d4031:

    # dd "No, the B-word. And then that bitch had the nerve to really rub it in Dahlia's face."
    dd "Нет, имя на Б. А потом у этой сучки хватило наглости рассмеяться Далии прямо в лицо."

# game/code/d08/d08s05.rpy:153
translate russian d08s05_dd_7238fc03:

    # mc "Oh no."
    mc "Ох, нет."

# game/code/d08/d08s05.rpy:154
translate russian d08s05_dd_954291b6:

    # sy "The Prom Queen got to pick her Prom King?"
    sy "Королева выпускного бала должна выбрать своего короля выпускного бала?"

# game/code/d08/d08s05.rpy:155
translate russian d08s05_dd_0fbe898e:

    # dd "Yes! So, she picked Dahlia's boy toy. Honestly, I'm not sure how anyone survived that."
    dd "Да! Итак, она выбрала мальчика Далии. Честно говоря, я не была уверена, как кто-то после этого сможет выжить."

# game/code/d08/d08s05.rpy:157
translate russian d08s05_dd_8911c944:

    # mc "Yeah, I mean Dahlia must have been crushed."
    mc "Да, думаю что Далия, должно быть, была раздавлена."

# game/code/d08/d08s05.rpy:158
translate russian d08s05_dd_83d7d13c:

    # sy "Or pissed."
    sy "Или разозлилась."

# game/code/d08/d08s05.rpy:160
translate russian d08s05_dd_170e43ed:

    # dd "Totally pissed. Or at least, that's what I imagine."
    dd "Сильно разозлилась. Или, по крайней мере, мне так показалось."

# game/code/d08/d08s05.rpy:161
translate russian d08s05_dd_84c724b5:

    # mc "You weren't there?"
    mc "Ты тоже там была?"

# game/code/d08/d08s05.rpy:163
translate russian d08s05_dd_0bdf3048:

    # dd "Nah. I was out partying with Samiya that night. We weren't interested in the prom."
    dd "Нет. В тот вечер я была на вечеринке с Самией. Выпускной нас не интересовал."

# game/code/d08/d08s05.rpy:164
translate russian d08s05_dd_2058c3a7:

    # sy "Makes sense."
    sy "Понимаю."

# game/code/d08/d08s05.rpy:166
translate russian d08s05_dd_fbe676e7:

    # dd "So, it's like 4am and Dahlia's boyfriend comes in and starts telling everyone about it."
    dd "Итак, примерно в 4 утра, парень Далии приходит и начинает рассказывать всем об этом."

# game/code/d08/d08s05.rpy:167
translate russian d08s05_dd_978f9da2:

    # dd "He goes through the whole story, from how he got picked for Prom King and how after he fucked that B-word and just went on and on..."
    dd "Он рассказывает всю историю, начиная с того, как его выбрали королем выпускного бала, и переходит к тому, как после этого, он трахнул эту шлюху на букву Б, он продолжал и продолжал говорить..."

# game/code/d08/d08s05.rpy:169
translate russian d08s05_dd_d1d905d4:

    # sy "Men are such assholes."
    sy "Парни такие придурки."

# game/code/d08/d08s05.rpy:170
translate russian d08s05_dd_3f3e8c36:

    # dd "Samiya walked up to his drunk ass and asked him what happened to his girlfriend."
    dd "Самия подошла к его пьяной заднице и спросила, что случилось с его девушкой."

# game/code/d08/d08s05.rpy:171
translate russian d08s05_dd_5bc70cfb:

    # sy "Dahlia."
    sy "Далией."

# game/code/d08/d08s05.rpy:173
translate russian d08s05_dd_9d5a9ec4:

    # dd "The dude totally forgot about her. When he remembered, he didn't seem to give a shit."
    dd "Чувак совершенно забыл о ней. Но даже, когда он вспомнил, ему, похоже, было наплевать."

# game/code/d08/d08s05.rpy:174
translate russian d08s05_dd_440ed36e:

    # sy "Such an asshole."
    sy "Такой мудак."

# game/code/d08/d08s05.rpy:176
translate russian d08s05_dd_8b530a58:

    # dd "Samiya knocked his ass out with one punch. Right there in front everybody."
    dd "Самия одним ударом вырубила эту задницу. Прямо там, на глазах у всех."

# game/code/d08/d08s05.rpy:177
translate russian d08s05_dd_698d7569:

    # mc "Wow."
    mc "Вау."

# game/code/d08/d08s05.rpy:179
translate russian d08s05_dd_0106c1b2:

    # dd "You know how I said Samiya never did anything herself? She would get someone else to do it for her?"
    dd "Помните, я говорила, что Самия никогда ничего не делала сама? Она заставляет кого-то другого делать всё за нее?"

# game/code/d08/d08s05.rpy:180
translate russian d08s05_dd_00765a01:

    # mc "Not really, but sure."
    mc "Не совсем, но конечно."

# game/code/d08/d08s05.rpy:182
translate russian d08s05_dd_1512a6e1:

    # dd "Not that night. Samiya and I and a couple guys went over to the gym-"
    dd "Но не в ту ночь. Мы с Самией и парой парней пошли в спортзал..."

# game/code/d08/d08s05.rpy:183
translate russian d08s05_dd_fda4f73e:

    # sy "Where Prom had been held?"
    sy "Где проходил выпускной бал?"

# game/code/d08/d08s05.rpy:185
translate russian d08s05_dd_652f3f87:

    # dd "Yeah, and that B-word's car was the only one still in the parking lot."
    dd "Да, и машина этой шлюхи на Б была единственной, которая еще стояла на парковке."

# game/code/d08/d08s05.rpy:186
translate russian d08s05_dd_c1dff962:

    # mc "Oh..."
    mc "Ох..."

# game/code/d08/d08s05.rpy:188
translate russian d08s05_dd_7519b5a8:

    # dd "I watched the guys smash up the car for a while, then Samiya lit a roadside flare and threw it through that bitch's windshield."
    dd "Какое-то время я смотрела, как парни разбивают машину, потом Самия зажгла сигнальную ракету и швырнула ее в лобовое стекло этой сучки."

# game/code/d08/d08s05.rpy:189
translate russian d08s05_dd_c1e7f53c:

    # mc "Damn."
    mc "Черт."

# game/code/d08/d08s05.rpy:191
translate russian d08s05_dd_c28fd025:

    # dd "Yeah. The guys ran off after that. Like literally ran away. Samiya and I just sat on the hood of her car watching that bitch's car burn until sunrise."
    dd "Да. После этого парни сбежали. В буквальном смысле. А мы с Самией просто сидели на капоте ее машины и смотрели до самого рассвета, как горит машина этой суки."

# game/code/d08/d08s05.rpy:193
translate russian d08s05_dd_6214cc14:

    # mc "Wow. That's some story. {w}Really different from what Dahlia told me the other day."
    mc "Вау. Вот это история. {w}Очень отличается от того, что рассказала мне Далия на днях."

# game/code/d08/d08s05.rpy:195
translate russian d08s05_dd_fd36e2df:

    # dd "Yeah, well. You know how it goes. {w}I guess Samiya didn't want anyone messing with Dahlia except for her."
    dd "Ну да. Ты знаешь, как это бывает. {w}Думаю, Самия не хотела, чтобы кто-нибудь связывался с Далией, кроме нее."

# game/code/d08/d08s05.rpy:197
translate russian d08s05_dd_74079c8b:

    # sy "Do you know where Samiya is? I kinda need to find her."
    sy "Ты знаешь, где Самия? Мне нужно найти ее."

# game/code/d08/d08s05.rpy:199
translate russian d08s05_dd_d8682bc6:

    # dd "Nah. We kinda drifted apart, ya'know? I'm sure she's somewhere around campus."
    dd "Неа. Мы вроде как разошлись, понимаешь? Я уверена, что она где-то в кампусе."

# game/code/d08/d08s05.rpy:201
translate russian d08s05_dd_a3efa3a7:

    # mc "Do you know anyone she does hang out with? Maybe someone else from your school?"
    mc "Ты знаешь кого-нибудь, с кем она тусуется? Может, кто-нибудь из вашей школы?"

# game/code/d08/d08s05.rpy:203
translate russian d08s05_dd_0c802f51:

    # dd "Oh, yeah! You were at that Party last week - did you meet Allison?"
    dd "О, да! Ты же был на той вечеринке на прошлой неделе - ты познакомился с Эллисон?"

# game/code/d08/d08s05.rpy:204
translate russian d08s05_dd_cb22420e:

    # mc "Allison?"
    mc "Эллисон?"

# game/code/d08/d08s05.rpy:206
translate russian d08s05_dd_133f952e:

    # sy "Allison?"
    sy "Эллисон?"

# game/code/d08/d08s05.rpy:207
translate russian d08s05_dd_7763faa5:

    # mc "Oh right! Allison!"
    mc "Ох, да! Эллисон!"

# game/code/d08/d08s05.rpy:238
translate russian d08s05_dd_fcbc2278:

    # mc "Of course, she gave me a tour of the place when I got there."
    mc "Конечно, она устроила мне экскурсию, когда я туда пришёл."

# game/code/d08/d08s05.rpy:240
translate russian d08s05_dd_94ac3014:

    # sy "Do you know all the girls in this city?"
    sy "Ты знаешь всех девушек в этом городе?"

# game/code/d08/d08s05.rpy:242
translate russian d08s05_dd_50a76bc7:

    # dd "Allison's a weird one. She didn't know any of us back in school, but she's been tight with Samiya here for some reason."
    dd "Эллисон странная. Она не дружила ни с кем из нас в школе, но по какой-то причине была близка с Самией."

# game/code/d08/d08s05.rpy:244
translate russian d08s05_dd_e721cf8a:

    # mc "Oh cool. Any idea where I can find Allison?"
    mc "Круто. Есть идеи, где я могу найти Эллисон?"

# game/code/d08/d08s05.rpy:245
translate russian d08s05_dd_3620c7bb:

    # dd "Sure. She's pretty much always at the pool."
    dd "Конечно. Она почти всегда в бассейне."

# game/code/d08/d08s05.rpy:246
translate russian d08s05_dd_949e911c:

    # mc "Thanks a lot!"
    mc "Большое спасибо!"

# game/code/d08/d08s05.rpy:248
translate russian d08s05_dd_e7148371:

    # dd "Ciao! And good luck!"
    dd "Чао! И удачи!"

# game/code/d08/d08s05.rpy:250
translate russian d08s05_dd_feb0aa65:

    # sy "Ok, let's go find another girl..."
    sy "Ладно, давай найдем эту девушку..."

# game/code/d08/d08s05.rpy:256
translate russian d08s05_dw_6d1a5426:

    # mc "See that woman over there? That's Dahlia."
    mc "Видишь вон ту девушку? Это Далия."

# game/code/d08/d08s05.rpy:258
translate russian d08s05_dw_0110ec89:

    # sy "You said she should be around her. I guess you were right."
    sy "Ты говорил, что она должна быть рядом. Думаю, ты был прав."

# game/code/d08/d08s05.rpy:265
translate russian d08s05_dw_5d822aea:

    # mc "Yep. {w}Hey there, Dahlia!"
    mc "Ага. {w}Привет, Далия!"

# game/code/d08/d08s05.rpy:266
translate russian d08s05_dw_0a84f66d:

    # dw "Oh, if it isn't my least favorite slave."
    dw "О, это же мой самый любимый раб."

# game/code/d08/d08s05.rpy:268
translate russian d08s05_dw_8ddece80:

    # sy "Did she just call you her slave?"
    sy "Она только что назвала тебя своим рабом?"

# game/code/d08/d08s05.rpy:269
translate russian d08s05_dw_c2ae4ef3:

    # mc "Heh... um..."
    mc "Хех... эм..."

# game/code/d08/d08s05.rpy:270
translate russian d08s05_dw_2fd7fd8a:

    # mct "This might not be as easy as I thought."
    mct "Это будет не так просто, как я думал."

# game/code/d08/d08s05.rpy:273
translate russian d08s05_dw_61e5b595:

    # mc "Dahlia! {w}Have you met my sister, Stacy?"
    mc "Далия! {w}Ты не знакома с моей сестрой, Стейси?"

# game/code/d08/d08s05.rpy:275
translate russian d08s05_dw_f867e91d:

    # mc "Dahlia! {w}This is Stacy. Stacy, this is Dahlia."
    mc "Далия! {w}Это Стейси. Стейси, это Далия."

# game/code/d08/d08s05.rpy:277
translate russian d08s05_dw_90e89a47:

    # dw "Hi Stacy. Would you enjoy a little roleplay?"
    dw "Привет, Стейси. Не хочешь немного поиграть в ролевые игры?"

# game/code/d08/d08s05.rpy:279
translate russian d08s05_dw_7e8b2165:

    # sy "Depends. Are you going to be the Dungeon Master or the Mistress?"
    sy "Зависит от того: Ты будешь Хозяйкой Подземелья или Госпожой?"

# game/code/d08/d08s05.rpy:281
translate russian d08s05_dw_e9f6f6e2:

    # dw "Very good. I like her."
    dw "Очень хорошо. Она мне нравится."

# game/code/d08/d08s05.rpy:282
translate russian d08s05_dw_d868b172:

    # mc "I'm... glad."
    mc "Я... рад."

# game/code/d08/d08s05.rpy:284
translate russian d08s05_dw_f61f66e0:

    # dw "Come now, don't tell me you've forgotten how to properly address me."
    dw "Только не говори, что забыл, как правильно обращаться ко мне."

# game/code/d08/d08s05.rpy:286
translate russian d08s05_dw_08f1b822:

    # mc "I'd love to play, but we're kinda in a hurry. Right, Stacy?"
    mc "Я бы с удовольствием поиграл, но мы немного торопимся. Верно, Стейси?"

# game/code/d08/d08s05.rpy:288
translate russian d08s05_dw_e0798176:

    # sy "Oh no, this sounds like too much fun. I wish I had some popcorn."
    sy "О нет, это звучит слишком весело. Жаль, что у меня нет попкорна."

# game/code/d08/d08s05.rpy:290
translate russian d08s05_dw_a8a9cf63:

    # dw "See? It's unanimous."
    dw "Видишь? Единогласно."

# game/code/d08/d08s05.rpy:292
translate russian d08s05_dw_7458e4ba:

    # mc "But, I don't want-"
    mc "Но я не хочу ... "

# game/code/d08/d08s05.rpy:294
translate russian d08s05_dw_5ee80470:

    # dw "No, no, no. Don't lie. You know you want to play."
    dw "Нет, нет, нет. Не ври. Ты же знаешь, что хочешь играть."

# game/code/d08/d08s05.rpy:296
translate russian d08s05_dw_4b5ff222:

    # mc "*sigh* Yes, Mistress."
    mc "*вздох* да, Госпожа."

# game/code/d08/d08s05.rpy:298
translate russian d08s05_dw_cfc5fbfb:

    # dw "That's a good start. Now, do it properly. On your knees."
    dw "Это хорошее начало. А теперь сделай это как следует. На колени."

# game/code/d08/d08s05.rpy:302
translate russian d08s05_dw_b5d6a09a:

    # mc "You can't mean for me to do this here... in public?"
    mc "Ты хочешь, чтобы я сделал это здесь... на публике?"

# game/code/d08/d08s05.rpy:303
translate russian d08s05_dw_9baeba78:

    # dw "Would you prefer that I make some noise and then spank you in front of a crowd?"
    dw "Ты бы предпочел, чтобы я поругала, а потом отшлепала тебя перед всеми?"

# game/code/d08/d08s05.rpy:304
translate russian d08s05_dw_766fbcc4:

    # mc "What? No!"
    mc "Что? Нет!"

# game/code/d08/d08s05.rpy:306
translate russian d08s05_dw_f49c51d0:

    # dw "Excuse me?"
    dw "Прошу прощения?"

# game/code/d08/d08s05.rpy:307
translate russian d08s05_dw_07dcbcad:

    # mc "I mean, please, no. Please, don't do that, Mistress."
    mc "То есть, пожалуйста, нет. Пожалуйста, не делайте этого, Госпожа."

# game/code/d08/d08s05.rpy:311
translate russian d08s05_dw_2231a2cf:

    # mc "Yes, Mistress. Please forgive me, Mistress."
    mc "Да, Госпожа. Пожалуйста, простите меня, Госпожа."

# game/code/d08/d08s05.rpy:313
translate russian d08s05_dw_8fe19cec:

    # dw "Better. Now kneel."
    dw "Уже лучше. А теперь встань на колени."

# game/code/d08/d08s05.rpy:316
translate russian d08s05_dw_e2f9c32f:

    # dw "Good, good. Are you enjoying yourself?"
    dw "Хорошо, очень хорошо. Тебе это нравится?"

# game/code/d08/d08s05.rpy:318
translate russian d08s05_dw_ee2aaa9b:

    # sy "Oh my, yes. Does he do any tricks?"
    sy "О боже, да. Он умеет делать какие-нибудь фокусы?"

# game/code/d08/d08s05.rpy:320
translate russian d08s05_dw_6bac721e:

    # dw "Stacy, was it? Did you want to be audience or participant?"
    dw "Стейси, правильно? Ты хотела бы быть зрителем или участником?"

# game/code/d08/d08s05.rpy:322
translate russian d08s05_dw_549e8ea5:

    # dw "There's plenty of room here for another slave kneeling before me."
    dw "Здесь достаточно места для еще одного раба, стоящего на коленях передо мной."

# game/code/d08/d08s05.rpy:328
translate russian d08s05_dw_e2f543a6:

    # sy "Audience. Definitely audience."
    sy "Зрителем. Определенно зрителем."

# game/code/d08/d08s05.rpy:330
translate russian d08s05_dw_999e1c24:

    # dw "Good, then shhh."
    dw "Хорошо, тогда шшш."

# game/code/d08/d08s05.rpy:332
translate russian d08s05_dw_fbbb0f24:

    # dw "So, is my slave to keep me waiting? Tell your Mistress if you are enjoying yourself."
    dw "Итак, мой раб заставит меня ждать? Скажи своей Госпоже, если тебе это нравится."

# game/code/d08/d08s05.rpy:334
translate russian d08s05_dw_7fd3a623:

    # mc "Yes, Mistress."
    mc "Да, Госпожа."

# game/code/d08/d08s05.rpy:335
translate russian d08s05_dw_0965319c:

    # dw "I thought so."
    dw "Я так и думала."

# game/code/d08/d08s05.rpy:337
translate russian d08s05_dw_975ccae0:

    # dw "Hmm... there's something wrong here."
    dw "Хмм... здесь что-то не так."

# game/code/d08/d08s05.rpy:338
translate russian d08s05_dw_c414faf3:

    # dw "Yes, yes. Something is definitely not proper about this."
    dw "Да, да. Что-то здесь определенно не так."

# game/code/d08/d08s05.rpy:340
translate russian d08s05_dw_0ba25fbe:

    # dw "Oh, I know what it is! What you're wearing is completely inappropriate."
    dw "О, я поняла, что не так! То, что на тебе надето, совершенно неуместно."

# game/code/d08/d08s05.rpy:341
translate russian d08s05_dw_4433c6e6:

    # mc "I'm sorry, Mistress."
    mc "Простите, Госпожа."

# game/code/d08/d08s05.rpy:343
translate russian d08s05_dw_6ec54438:

    # dw "Hmm... Stacy, how should he correct his apparel?"
    dw "Хмм... Стейси, как он должен исправить свою одежду?"

# game/code/d08/d08s05.rpy:345
translate russian d08s05_dw_7eef1638:

    # sy "Oh, well... {w}Clearly he should be barefoot."
    sy "Ну... {w}Очевидно, что он должен быть бос."

# game/code/d08/d08s05.rpy:347
translate russian d08s05_dw_ae3b6be8:

    # dw "Excellent suggestion."
    dw "Отличное предложение."

# game/code/d08/d08s05.rpy:349
translate russian d08s05_dw_4cd8fefd:

    # dw "Slave, take off your shoes."
    dw "Раб, сними обувь."

# game/code/d08/d08s05.rpy:350
translate russian d08s05_dw_27bd83cd:

    # mct "At least this can't get much more embarrassing... can it?"
    mct "По крайней мере, это не должно быть неловко... Правда?"

# game/code/d08/d08s05.rpy:351
translate russian d08s05_dw_7fd3a623_1:

    # mc "Yes, Mistress."
    mc "Да, Госпожа."

# game/code/d08/d08s05.rpy:356
translate russian d08s05_dw_333e1ebd:

    # dw "Now slave, think back to our previous interactions. How should you be dressed in front of me?"
    dw "А теперь, раб, вспомни нашу предыдущую встречу. Как ты должен быть одет передо мной?"

# game/code/d08/d08s05.rpy:357
translate russian d08s05_dw_ba203ecd:

    # mct "What? She can't be serious."
    mct "Что? Она это не серьезно."

# game/code/d08/d08s05.rpy:359
translate russian d08s05_dw_2496bc44:

    # dw "I'm waiting..."
    dw "Я жду..."

# game/code/d08/d08s05.rpy:361
translate russian d08s05_dw_7d0f621d:

    # mc "I'm unsure, Mistress."
    mc "Я не уверен, Госпожа."

# game/code/d08/d08s05.rpy:363
translate russian d08s05_dw_16fed8ed:

    # dw "I doubt that. Well, if you don't know then you may beg me to remind you."
    dw "Я в этом сомневаюсь. Ну, если ты не помнишь, то можешь попросить меня напомнить тебе."

# game/code/d08/d08s05.rpy:364
translate russian d08s05_dw_522460b0:

    # mct "Oh crap. She wants me to beg her to tell me to strip naked right here in public while Stacy is watching?"
    mct "Вот дерьмо. Она хочет, чтобы я умолял ее разрешить мне раздеться догола прямо здесь, на публике, перед Стейси?"

# game/code/d08/d08s05.rpy:366
translate russian d08s05_dw_80a5c16f:

    # mc "Umm..."
    mc "Эмм..."

# game/code/d08/d08s05.rpy:368
translate russian d08s05_dw_da6a3daa:

    # sy "I hate to interrupt. This is a lot of fun, but-"
    sy "Я не хочу прерывать вас. Это очень весело, но ..."

# game/code/d08/d08s05.rpy:369
translate russian d08s05_dw_662a103c:

    # dw "But what?"
    dw "Но что?"

# game/code/d08/d08s05.rpy:371
translate russian d08s05_dw_953507d0:

    # sy "We're actually here to get my stuff back. Do you know where we can find Samiya?"
    sy "Вообще-то мы здесь, чтобы забрать мои вещи. Ты не знаешь, где мы можем найти Самию?"

# game/code/d08/d08s05.rpy:372
translate russian d08s05_dw_4b0fc891:

    # dw "Ugh. Her, again?"
    dw "Агх. Опять она?"

# game/code/d08/d08s05.rpy:374
translate russian d08s05_dw_a0a7a5a7:

    # sy "She stole my luggage."
    sy "Она украла мой багаж."

# game/code/d08/d08s05.rpy:376
translate russian d08s05_dw_bda7583a:

    # dw "Whatever... {w}Fine. I'll tell you where you can find someone who will know where she is."
    dw "Неважно... {w}Ладно. Я скажу, где вы можете найти кого-нибудь, кто будет знать, где она."

# game/code/d08/d08s05.rpy:377
translate russian d08s05_dw_b3e5d52d:

    # sy "Can't you just-"
    sy "Разве ты не можешь просто..."

# game/code/d08/d08s05.rpy:380
translate russian d08s05_dw_317588c3:

    # mct "What the fuck??"
    mct "Какого хрена??"

# game/code/d08/d08s05.rpy:382
translate russian d08s05_dw_ebf64c4f:

    # dw "Well, first of all, I don't care where that bitch is these days. As long as she stays away from me."
    dw "Ну, во-первых, мне все равно, где сейчас эта сука. Пока она держится от меня подальше."

# game/code/d08/d08s05.rpy:383
translate russian d08s05_dw_ae352a33:

    # dw "Second, I wouldn't tell you even if I knew where she was."
    dw "Во-вторых, я не сказала бы вам, даже если бы знала, где она."

# game/code/d08/d08s05.rpy:385
translate russian d08s05_dw_74abaaf7:

    # sy "Why not?"
    sy "Почему нет?"

# game/code/d08/d08s05.rpy:387
translate russian d08s05_dw_882f79fd:

    # dw "Don't you get it? She's bad news. Forget about your missing stuff and forget about finding her."
    dw "Ты что, не понимаешь? Она - плохая новость. Забудь о своих пропавших вещах и забудь о том, чтобы найти ее."

# game/code/d08/d08s05.rpy:389
translate russian d08s05_dw_9581568c:

    # sy "Not gonna happen."
    sy "Этого не случится."

# game/code/d08/d08s05.rpy:391
translate russian d08s05_dw_44fd1839:

    # dw "Which brings us to what I am willing to tell you - if you two will listen."
    dw "И это подводит нас к тому, что я хочу вам сказать - если вы будете слушать."

# game/code/d08/d08s05.rpy:393
translate russian d08s05_dw_f4a296e2:

    # mc "Ugh.. Can I..."
    mc "Агх... Могу я..."

# game/code/d08/d08s05.rpy:396
translate russian d08s05_dw_dd5eddfc:

    # sy "I'm all ears!"
    sy "Я вся внимание!"

# game/code/d08/d08s05.rpy:398
translate russian d08s05_dw_2f9aed17:

    # dw "[mcname] should have met someone last week who would know Samiya's whereabouts."
    dw "[mcname] должен был встретиться на прошлой неделе с кем-нибудь, кто бы знал, где находится Самия."

# game/code/d08/d08s05.rpy:399
translate russian d08s05_dw_c678c30c:

    # mct "I did?"
    mct "С кем это?"

# game/code/d08/d08s05.rpy:400
translate russian d08s05_dw_d661b00b:

    # dw "If he has any brain he can remember Allison. The girl who was always at the pool..."
    dw "Если у него есть мозги, то он может вспомнить Эллисон. Девушку, которая всегда находится у бассейна..."

# game/code/d08/d08s05.rpy:402
translate russian d08s05_dw_6d8d2ea3:

    # mc "Ugh... yeah..."
    mc "Агх... Да..."

# game/code/d08/d08s05.rpy:404
translate russian d08s05_dw_9ebf841f:

    # sy "Any ideas where we could find her?"
    sy "Есть идеи, где мы могли бы ее найти?"

# game/code/d08/d08s05.rpy:406
translate russian d08s05_dw_dbaf6043:

    # dw "Of course. She's always at the pool."
    dw "Конечно. Она всегда у бассейна."

# game/code/d08/d08s05.rpy:409
translate russian d08s05_dw_73f71408:

    # mc "What the fuck was that?!"
    mc "Какого хрена это было?!"

# game/code/d08/d08s05.rpy:411
translate russian d08s05_dw_f73ca9ad:

    # sy "We were getting information. It was your plan, wasn't it?"
    sy "Мы получали информацию. Это же был твой план, верно?"

# game/code/d08/d08s05.rpy:412
translate russian d08s05_dw_8a87e0d1:

    # mc "You didn't need to sit on me!"
    mc "Тебе не нужно было садиться на меня!"

# game/code/d08/d08s05.rpy:414
translate russian d08s05_dw_1e4b29aa:

    # sy "I was streamlining the process... it worked, right?"
    sy "Я упростила этот процесс... это же сработало, верно?"

# game/code/d08/d08s05.rpy:415
translate russian d08s05_dw_1d0e9828:

    # sy "Let's go find this Allison."
    sy "Давай найдем эту Эллисон."

translate russian strings:

    # game/code/d08/d08s05.rpy:299
    old "In public?!?"
    new "На публике?!?"

    # game/code/d08/d08s05.rpy:299
    old "Yes, Mistress"
    new "Да, Госпожа"

# TODO: Translation updated at 2021-04-10 22:37

# game/code/d08/d08s05.rpy:38
translate russian d08s05_b4dc6ce9:

    # sy "So... What's the plan?"
    sy "So... What's the plan?"

